...
%     % \end{beamercolorbox}%
%     \addalert{\usebeamerfont{frametitle} \footnotesize \textbf{Comment valoriser l’énergie solaire pour des MEPOS ?}}
%     \vfill
%     \begin{columns}
...
%                     \item<7->[--] Recherche d’alternatives pertinentes.
%                     \item<7->[--] Aide au choix de solutions interactivement.
%                 \end{itemize}
%             \end{center}
%         \end{column}%
%     \end{columns}%
...
\begin{frame}[plain]
    ...
    \centering
    \begin{beamercolorbox}[sep=8pt,center,shadow=true,rounded=true]{title}
    \usebeamerfont{title}Presentation de la problématique traitée\par%
    \end{beamercolorbox}
    \vfill
\end{frame}
...
\begin{frame}[t]
    ...
    \small
    \begin{itemize}
        \item<1-> Multi-critères et multi-physiques.
        \vfill
        \item<2-> Quelle performance SSC + MEPOS ?
        \vfill
        \item<3-> Tenir compte de la géométrie et de la surface limitée de toiture.
        \vfill
        \item<3-> Équilibre Besoins / Capacité de production / Consommations ?
    \end{itemize}
    \vfill
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{columns}
        \begin{column}{0.2\textwidth}
            \vskip3.5em
            \uncover<5->{\addsubtitle{Objectifs}}
            \vskip7em
        ...
        \begin{column}{0.8\textwidth}
            \uncover<2->{%
                \begin{itemize}
                    \item \only<2-4>{Évaluer la performance du SSC} \only<5->{Maximiser la performance du SSC}
                \end{itemize}
                \begin{columns}
            ...
            \uncover<7->{%
                \begin{itemize}
                    ...
                    \item<8-> \footnotesize $\abs{Prod_{PV} - Conso_{usages} - Conso_{app}}  \geq  8 \si{kWh_{ep}\per\metre\squared}$
                \end{itemize}
            }
        \end{column}
    \end{columns}
...
\begin{frame}[c]
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            ...
            \vskip0.2em
            \includegraphics[width=1.2\textwidth, clip=true, trim=30mm 70mm 5mm 15mm]{Ressources/Templates/Optimisation/Opposite_based_learning/principe_obl.pdf}
            \vskip2em
            }
            ...
            \begin{itemize}
                ...
                \item \alt<6->{\colorbox{SolarizedRed}{Vol de Lévy}}{Vol de Lévy}
            \end{itemize}
            \vskip2em
            }
...
\begin{frame}[t]
    ...
    \vskip0.5em
    \vfill
    \uncover<3->{\addalert{Est-il nécessaire de faire varier tous les paramètres~?}}
\end{frame}
...
\begin{frame}[t]
    ...
    % \only<4->{\vfill \includegraphics[width=1.05\textwidth, clip=true trim0mm 0mm 0mm 2mm]{Ressources/Images/Modelisation/Captures/SSC_capture.pdf}\vfill}
\end{frame}
...
\begin{frame}[t]
    \begin{adjustwidth}{-2em}{-2em}
        \includegraphics[height=0.8\textheight, clip=true trim0mm 0mm 0mm 2mm]{Ressources/Images/Modelisation/Captures/SSC_capture.pdf}
    \end{adjustwidth}
    \vfill
    \uncover<2->{%
        \addalert{\small Temps de simulation important}
    }
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            ...
            % \raggedright
            % \begin{equation*}
            %     \mathcal{M}(\,\underline{\vec{x}}\,) \approx \sum_{\abs{\vec{\alpha}} \leq P} c_{\vec{\alpha}} \times \psi_{\vec{\alpha}}(\,\underline{\vec{x}}\,)
            % \end{equation*}
            % \begin{equation*}
            %     \min(Y - \mathcal{M}(\,\underline{\vec{x}}\,))
            % \end{equation*}
            }
        \end{column}
    \end{columns}
...
\begin{frame}[c]
    ...
    \begin{tabular}{l c c c c c c c c}
        \toprule
                        ...
                        & & & min & max   &       & & min & max \\
        \midrule
        \multirow{3}{*}{Référence (\si{kWh})} & ECS & &  \multicolumn{2}{c}{2588}  & &  & \multicolumn{2}{c}{2866}  \\
        \addlinespace[\defaultaddspace]
                                   & CH  & &  \num{228}  & \num{620} & &  & \num{1363}   & \num{2110} \\
        \addlinespace[\defaultaddspace]
                                   & ECS + CH & &  \num{2816} & \num{3208} & & & \num{4229}   & \num{4976} \\
        \\
        \addlinespace[\defaultaddspace]
        \uncover<2->{\multirow{3}{*}{Appoint (\si{\percent})} & $F_{sav}^{ECS}$ & &  \num{64} & \num{94} & & & \num{58}   & \num{89}} \\
        \addlinespace[\defaultaddspace]
                                                 & \uncover<2->{$F_{sav}^{CH}$  & &  \num{22}  & \num{84} & &  & \num{8}   & \num{62}} \\
        \addlinespace[\defaultaddspace]
                                                 & \uncover<2->{$F_{sav,\,ext}$ & &  \num{57} & \num{90} & & & \num{42}   & \num{72}} \\
        \\
        \addlinespace[\defaultaddspace]
        \uncover<3->{Appoint (\si{kWh}) & ECS + CH & &  \num{324} & \num{1372} & & & \num{1222}   & \num{3117}} \\
        \\
        \addlinespace[\defaultaddspace]
        \multicolumn{2}{c}{\uncover<4->{Production photovoltaïque (\si{kWh})}}       & & \uncover<4->{\num{3239} & \num{4743} & & & \num{4014}   & \num{6021}} \\
        \bottomrule
    \end{tabular}
    \end{table}
    \vfill
...
\begin{frame}[c]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item<5->[--] Exploration automatisée de l’espace de décision.
            \item<5->[--] Grande diversité de solutions optimales obtenue.
            \item<5->[--] Aide à la décision simple et interactive.
        \end{itemize}
        }
        \vfill
