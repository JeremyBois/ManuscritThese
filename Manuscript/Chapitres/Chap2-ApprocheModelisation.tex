...
débit hygiénique ($Q_{min} = $\SI[per-mode=symbol]{90}{\meter\cubed\per\hour}) peut être insuffisant. Une
...
de besoin avec un débit maximal de $Q_{max} = $\SI[per-mode=symbol]{900}{\meter\cubed\per\hour}. Ces
...
\SI{33}{\litre\per(jour{.}pers)}~(\SI{60}{\celsius}), soit un
...
consommations de 27, 33, et \SI{40}{\litre\per(jour {.} pers)}~(\SI{60}{\celsius}) seront
aussi évaluées.
...
\begin{itemize}
    \item Cas de référence~: \SI[per-mode=symbol]{90}{\meter\cubed\per\hour} (\ref{ssub:ventilation_ref}).
...
\begin{tabular}{l | c | ccc}
    ...
    $R_{optique}$                            & \si{\%}                 & \num{78}             & \num{83.4}           & \num{63}        \\
    Décroissance de performance              & -                       & \num{-5.103}         & \num{-4.777}         & \num{-0.975}    \\
    Coefficient $a_{1}$ (pertes linéiques)   & \si{W/(m^{2}{.}K)}      & \num{3.796}          & \num{1.4539}         & \num{0.9249}    \\
    Coefficient $a_{2}$ (pertes surfaciques) & \si{W/(m^{2}{.}K^{2})}  & \num{0,013}          & \num{0.0589}         & \num{0.00069}   \\
...
été retenue de moduler le débit nominale par capteur (\SI[per-mode=symbol]{}{\litre\per(\meter\squared\period\hour)}).
