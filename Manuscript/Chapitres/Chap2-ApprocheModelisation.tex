...
Ces travaux cherche à évaluer la performance d’un \abr{SSC} à l’échelle d’une maison
...
\label{par:trnssys}
Aujourd’hui en version $18$ \parencite{Klein2017} il est développée par l’université de Madison (\abr{USA}).
...
\fnref{www.retscreen.net}{\textit{RETScreen}}. Les résultats montrent la performance
...
d’un \abr{SSC} (vecteur air ou eau) ou bien d’une système produisant uniquement de
...
besoins d’\abr{ECS} sont important en particulier pour des capteurs sous-vide. Cependant
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Buildings and BuildingsPy} % (fold)
\label{ssub:buildings_and_buildingspy}

% subsubsection buildings_and_buildingspy (end)
...
\itodo{Trouver un moyen de rendre clair la comparaison}

\begin{figure}
    \centering
    \includegraphics{Ressources/Images/Modelisation/Batiment/compare_power.pdf}
...
et \SI{117}{\watt} par adulte(\tabref{tab:puissance_occupants}).
...
% ------------------------------------------------------------------------------
\itodo{Mettre des capture du modèle \textit{Modelica} (système et algorithme)}
...
Ces travaux s’inscrivent dans le cadre du projet \abr{COMEPOS} (\ref{ssub:le_projet_comepos})
...
couvrir les besoins en \abr{ECS} et en chauffage (\abr{CH}) pour l’ensemble du bâtiment.
...
\begin{table}
\centering
\caption{Caractéristiques du collecteur de référence (\textit{Sonnenkraft IDMK\,25-AL}) d’après \figref{fig:caracs_idmk}.}
...
\paragraph{Calcul des \abr{IAM}~:} % (fold)
...
\begin{figure}
    \centering
    \includegraphics{Ressources/Images/Modelisation/Capteurs/IDMK/IAM.pdf}
...
(\figref{fig:compare_capteurs}, gauche). Enfin les résultats fournies par le modèle
\textit{Buildings} permettant de calculer les part respectives $I_{dif,\,inc}$ et $I_{dir,\,inc}$
sur une surface inclinée est aussi comparé avec les résultats obtenus dans \textit{TrnSys}.
Les deux modèles estiment de manière similaire l’irradiation sur une
...
modèles de la bibliothèque \fnref{https://github.com/lbl-srg/modelica-buildings}{\textit{Buildings}}
...
\begin{blockdescription}{Combles et Vide sanitaire}
    ...
    \item [$\bm{G_{dif,\,inc}}$] \textit{BoundaryConditions.SolarIrradiation.DiffusePerez} permettant
          de calculé le rayonnement diffus suivant le modèle développé par \textcite{Perez1990271}.
    \item [$\bm{G_{dir,\,inc}}$] \textit{BoundaryConditions.SolarIrradiation.DirectTiltedSurface}
          permettant de calculer le rayonnement direct selon \eqref{eq:rayonnement_surf_incline}.
...
($\Delta T_{sol}$). Contrairement aux approches plus classiques, la différence de
...
grâce à un \abr{PID} associé à chaque pompe.
...
à $\nicefrac{1}{6}$ de la position haute de l’échangeur solaire.

\begin{figure}
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \ftodo{Ajouter représentation schématique avec style controleur hystérésis}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \ftodo{Ajouter représentation schématique avec style controleur on off}
    ...
    \caption[Types de Contrôleurs]
             {Principe de fonctionnement d’un contrôleur \enquote{Hystérésis} (a)
              et d’un contrôleur \enquote{OnOff} (b).}
...
L’approche est donc robuste et évite des comportements éradiques pour des configurations
non prévues~: l’algorithme est donc plus stable.
...
l’algorithme. Il existe toujours un unique état maître mais celui ci peut avoir
...
\begin{figure}
    ...
    \caption{Description détaillée de l’\abr{HFSM} contrôlant la transition entre
...
contrôle des équipements à une combinaison de \abr{PID} fonctionnant par rétroaction (feed-back).
...
dans le cadre d’un travail en collaboration avec l’entreprise \abr{IGC} à travers le projet
\abr{COMEPOS} et est le fruit d’un travail itératif. Le bâtiment comme le \abr{SSC} ont
...
% Enfin les modèles retenus ont une surface nette similaire facilitant la comparaison entre
% eux.

% \begin{table}
% \centering
% \caption{Caractéristiques des capteurs solaires.
...
\begin{table}
\centering
\itodo{Ajouter variations algorithmiques}
...
\itodo{Mettre en avant la diversité des climats}
...
de la production d’\abr{ECS} est presque nulle. À l’opposé sur Strasbourg, quatre capteurs
...
% \itodo{Ajouter couverture mensuelle en fonction des climats}
...
\abr{SSC} (technique et algorithmique) il est n’est pas nécessaire de simuler les périodes ou
...
supérieures car ils tiennent compte de la période d’autonomie complète du \abr{SSC}.
...
détriment de l’\abr{ECS} expliquant cet équilibre. Il est aussi intéressant de noter le
...
simulations faisant varier le volume puisé montre que le \abr{SSC} est robuste. Il est en
...
% Les résultats montrent aussi que le capteur plan vitrée du
...
\itodo{Revoir le blabla}
Au cours de ce chapitre, la démarche entreprise a été explicitée. Dans l’optique de mieux
comprendre les interactions entre le bâtiment et le \abr{SSC}, un modèle mixte a été réalisé à
l’aide du langage \textit{Modelica}. Pour le bâtiment, un modèle mono-zone a été retenu
suite à une comparaison avec une approche plus détaillée. Pour le \abr{SSC}, une approche
...
fonctionnement couplé du bâtiment et du \abr{SSC}. Finalement, une étude paramétrique a permis
de mieux comprendre le comportement du \abr{SSC}.
...
besoins en \abr{ECS} comme les constructions performantes actuelles le suggère.
...
$Conso_{app}$ pour le chauffage et la production d’\abr{ECS}. Uniquement considérer la
$Conso_{app}$ ne permet en effet pas de tenir compte des influences bidirectionnelles
entre le \abr{SSC} et le bâtiment illustrées à travers l’étude paramétrique. Dans l’optique
