...
\itodo{Corriger les nombre décimaux . --> ,}
...
Enfin, le système comporte 3 pompes~: S6, S5, et S2. Il est considéré un débit nominale de \SI{40}{\liter\per(\meter\squared{.}\hour}
pour les pompes S6 et S5 et de \SI{70}{\liter\per(\meter\squared{.}\hour} pour la pompe S2 comme
...
empiriques décrites dans le volet 2 de la norme EN12975 (\mtodo{Ajouter Ref}{EN12975-2}).
...
% ------------------------------------------------------------------------------
\subsection{Équilibrage du réseau} % (fold)
\label{sub:equilibrage_du_reseau}
~
\itodo{Dans étude paramétrique}
Le système décrit est complexe pour plusieurs raisons~:
\begin{itemize}
  \item il comporte de nombreux mode et état de fonctionnements
  \item le sens de circulation du fluide est variable
  \item plusieurs pompes en série peuvent être active en même temps
\end{itemize}

Le dernier élément est particulièrement important car il a un impact direct sur le
calcul du débit total lorsque deux pompes en série sont actives. Par défaut, les
modèles hydrauliques de la bibliothèque \textit{Buildings} considère un cas idéal
sans pertes de charges.
Ainsi si on considère deux pompes en série ayant toutes les deux pour consigne un débit de \SI{2}{\meter\cubed\per\hour}
le débit résultant sera de \SI{4}{\meter\cubed\per\hour}. Afin de corriger ce
comportement, il est nécessaire d’équilibrer l’ensemble du réseau et de sélectionner
une pompe en fonction des pertes de charges à couvrir.

\itodo{Liste les variations retenues}
Cependant dans cette étude paramétrique, l’influence du débit nominale sur la performance
du système solaire est aussi évaluée~:
...
durant l’initialisation du modèle. Dans un premier temps la surface totale de capteur est
calculée permettant de calculer le débit nominal de chaque pompe. Ensuite les pertes
de charges sont réparties en suivant les règles décrites à travers le schéma \mtodo{Ref fig pdc}.

\ftodo{Ajouter figure décrivant équilibrage du réseau}


Une pompe à vitesse variable (adaptée aux températures élevées d’un réseau solaire) couvrant
l’ensemble des variations des pertes de charges du réseau a été sélectionnée (\mtodo{Ajouter ref carac pompe}).
% subsection equilibrage_du_réseau (end)
...
comme de la quantité des besoins sur la performance du système solaire combiné
(\emph{SSC}). Les consommations variant fortement d’une habitation à une autre, des
consommations de 27, 33, et \SI{40}{\liter\per(jour {.} pers)}~(\SI{60}{\celsius}) seront
aussi évaluées.
...
\begin{tabular}{| l | c | c | c | c |}
    ...
    \hline
Occupation (diurne)    &                     &                     & \cellcolor{black!30} & \cellcolor{black!30} \\
    \hline
Occupation (nocturne)  &                     & \cellcolor{black!30} & \cellcolor{black!30} & \cellcolor{black!30} \\
    \hline
Inoccupation         & \cellcolor{black!30} &                     & \cellcolor{black!30} &                     \\
    \hline
\end{tabular}
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Capteurs solaires thermiques} % (fold)
...
(\ref{ssub:capteurs_solaires_thermiques}). Les deux autres types sont issue d’une source
...
% subsubsection capteurs_solaires_thermiques (end)
...
Ensuite l’impact de la position de l’échangeur solaire est aussi étudiée.
L’échangeur du ballon de stockage couvrant presque l’intégralité de sa hauteur
seule la position de l’échangeur solaire sur le ballon sanitaire est analysée.
Afin d’être compatible avec les variations réalisées sur le volume des ballons,
la variation de la position de l’échangeur sera réalisée relativement à sa
position d’origine sans modifier sa longueur. Il est alors possible d’évaluer
l’impact du volume du ballon et de la position relative de l’échangeur de
manière combinés ou bien séparément. Les coefficients ont été sélectionnés en
fonction des limitations techniques des ballons qui imposent ne hauteur minimale
et maximale car la hauteur de l’échangeur est fixe. Les coefficients retenues
sont alors les suivants : 0.8, 1, et 1.3. Avec un coefficient de 0.8 l’échangeur
sera ainsi positionné plus bas et avec un coefficient de 1.3, il sera à l’inverse plus haut.
% subsubsection ballons (end)


% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Autres variations étudiées} % (fold)
\label{ssub:autres_variations_etudiees}
\paragraph{Sélection des pompes} % (fold)
\label{par:selection_des_pompes}
\itodo{Debit pompe, tempo, ...}


% paragraph selection_des_pompes (end)
% subsubsection autres_variations_etudiees (end)
