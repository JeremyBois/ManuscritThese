...
L’installation d’une expérimentation nécessite un investissement important alors que pour la
...
L’expérimentation de par ces contraintes, ne permet donc pas de répondre à la
problématique et l’approche par modélisation est donc le choix le plus pertinent.
...
\end{itemize}
De part son approche modulaire, le logiciel peut être étendus simplement afin de répondre aux
...
les développements continus dans le cadre du \fnref{https://ibpsa.github.io/project1/}{projet $1$}.
...
\label{ssub:dymtk}
\itodo{Ajouter code sur Github pour mettre le lien}
...
\itodo{Trouver un moyen de rendre clair la comparaison et ajouter blabla}
...
% ------------------------------------------------------------------------------
\itodo{Mettre des captures du modèle \textit{Modelica} (système et algorithme)}
...
s’inscrivent dans le cadre du projet \abr{COMEPOS} (\ref{ssub:le_projet_comepos})
...
simultanément. Ainsi, si on considère deux circulateurs en série ayant tous les deux pour
...
vitesse de rotation, soit un différentiel de pression. Le débit est ainsi calculé
en tenant compte des pertes de charges du réseau et de la vitesse maximale de rotation
...
températures élevées d’un réseau solaire est retenu dans la fiche
technique est disponible en annexe \figref{fig:caracs_pompes}).
...
par les données expérimentales (\figref{fig:compare_capteurs}, gauche). Enfin les
résultats fournies par le modèle \textit{Fluid.SolarCollectors.EN12975} permettant de
calculer les part respectives $I_{dif,\,inc}$ et $I_{dir,\,inc}$ sur une surface inclinée
est aussi comparé avec les résultats obtenus dans \textit{TrnSys}. Les deux modèles
estiment de manière similaire l’irradiation sur une surface inclinée
(\figref{fig:compare_capteurs}, droite).
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics{Ressources/Images/Modelisation/Regulation/hysteresis.pdf}
        \caption{}
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \centering
        \includegraphics{Ressources/Images/Modelisation/onOff_control.pdf}
        \caption{}
        \label{fig:onOff}
    \end{subfigure}
...
\begin{figure}
    \centering
    \includegraphics[clip=true, trim=2mm 2mm 2mm 20mm]{Ressources/Images/Modelisation/Batiment/France_map.pdf}
...
importants pour la production d’\abr{ECS} et de \abr{CH}, l’ensoleillement plus important
sur Limoges permet de valoriser une plus grande part solaire
(\tabref{tab:description_climat}). La même remarque peut être faite au niveau du
...
Strasbourg, il semble que quatre capteurs solaires soient insuffisant pour être
majoritairement couvert par le solaire Finalement, le système solaire obtient une bonne
...
systèmes et bâtiment a été modélisé avec le langage \textit{Modelica}
Pour le bâtiment, un modèle mono-zone a été retenu
...
fonctionnement couplé du bâtiment et du \abr{SSC}. Finalement, une étude paramétrique a permise
...
$Conso_{app}$ ne permet en effet pas de tenir compte des influences réciproques
