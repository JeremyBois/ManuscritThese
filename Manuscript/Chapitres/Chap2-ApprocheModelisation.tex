...
\ftodo{Ajouter acausal vs causal}
...
autonomes.
\ftodo{Ajouter example modélisation}
...
La maison (\mtodo{ref image}) a une surface habitable de \SI{98.4}{\meter\squared}
et est de plain-pied \ftodo{Ajouter plan de la maison}.
...
comme des conditions limites (\mtodo{Réf fig}). Le modèle utilisé est une version
...
leur orientation (\mtodo{Ajouter ref}) et une description détaillée et exhaustive est
disponible en annexe (\mtodo{ref tableau annexe}).
\ttodo{Tableau décrivant les parois}
...
de chaque pièce du modèle multi-zone (\mtodo{ref tab}).

\ftodo{Profil d’occupation retenue}

\ttodo{Récapitulatif des puissances dissipées en fonction des pièces}
...
profil d’occupation (\mtodo{ref occupation}).
...
Le système modélisé (\mtodo{Ref figure}) est composé de 3 parties principales : la partie
hydraulique, la partie aéraulique, et la partie contrôle qui orchestre le fonctionnement
couplé entre le système solaire et le bâtiment (\mtodo{ref fig}).

\ftodo{Schéma du système, partie hydraulique et aéraulique}
...
déperditions sont ainsi considérées comme des charges internes au bâtiment. Afin de garantir le
maintien du confort thermique des occupants, un appoint électrique en partie haute du
ballon sanitaire est aussi ajouté. Ce dernier n’est activé que lorsque l’énergie
solaire disponible n’est pas suffisante. L’énergie du soleil est quand à elle transmise à l’eau par
l’intermédiaire de panneaux solaires puis vers le système aéraulique à l’aide d’un
...
sur l’air extrait (\mtodo{Ajouter ref partie aéraulique}). L’air soufflé est un mélange entre de l’air neuf et de l’air repris
...
\ftodo{Ajouter partie aéraulique}
...
chaque composant. Le système solaire fonctionne suivant trois principaux modes. Afin de
valoriser l’énergie solaire les différents modes sont ordonnées (\mtodo{Ajouter ref fig}).
La \textbf{priorité} revient au maintien du ballon sanitaire, au respect de la température
de consigne, puis à l’élévation de la température dans le ballon de stockage. Le respect
de ces priorité n’est pas pour autant restrictif : le système peut remplir plusieurs
fonctions en parallèles. Lorsque l’énergie solaire disponible est suffisante, le
chauffage, l’\emph{ECS}, et la charge du ballon tampon peuvent être réalisées de manière
simultanées.
\ftodo{Ajouter description FSM du chauffage}
...
de noter que les besoins en énergie peuvent être couverts de manière simultanée par
...
L’intervalle de température (55 - 60) représente alors une zone morte (\mtodo{ref fig}).
\ftodo{Ajouter diagramme expliquant hystérésis}
...
garantir le confort thermique (\mtodo{ref fig}).
\ftodo{Ajouter figure décrivant fonctionnement du système de chauffage}

Afin d’améliorer la couverture solaire sur le chauffage, l’algorithme autorise la
surchauffe de la maison durant les périodes diurnes d’inoccupation (\mtodo{ref fig algo}).
...
$T_{cons}^{solaire}$. Seule l’utilisation de l’énergie solaire $Directe$ est permis afin
d’augmenter la température intérieure. L’augmentation de la température intérieure permet
de profiter de l’inertie du bâtiment et de retarder, voir éviter la relance du chauffage
et donc l’utilisation potentielle de l’appoint électrique (\mtodo{ref fig}).
\ftodo{Ajouter figure décrivant le principe de la surchauffe diurne}
...
% ------------------------------------------------------------------------------
\subsection{Scénarios envisagés} % (fold)
...
mensuelle, et est extrapolée durant la simulation (\mtodo{ref tab}). On remarque une forte
...
\ttodo{Ajouter tab temp en fonction du climat}

\paragraph{Profils retenus :} % (fold)
\label{par:profils_retenus}
Cinq scénarios ont été ont été considérés dans ces travaux (\mtodo{ref fig}). Pour
...
journaliers sont pondérées en fonction du jour de la semaine (\mtodo{ref tab}).
\ftodo{Ajouter les profils de puisage}
...
la période hivernale que pendant la période estivale (\mtodo{ref tab}).
\ttodo{Tableau des coefficients hebdomadaires}
\ttodo{Tableau des coefficients mensuels}
...
% subsubsection description_des_composants (end)
% subsection systeme_hydraulique (end)
