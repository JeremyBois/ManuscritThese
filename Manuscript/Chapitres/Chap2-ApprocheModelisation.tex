...
L’installation d’une expérimentation nécessite un investissement important là ou pour la
modélisation seul le coût du développement des modèles est considéré. Cette investissement
...
est nécessaire de modifier la configuration de base afin d’étudier une variation ... Il
...
des résultats. Il est alors nécessaire de récolter des informations sur l’environnement,
...
l’ajout de capteurs. Ces capteurs existe en plusieurs modèles et certaines variables sont
...
température d’une maison sur une année nécessiterais une année complète
d’expérimentations. Au niveau numérique, une étude sur une année entière est facilité.
Grâce aux données recueillis dans le passé, il est possible de simuler l’évolution de la
température du bâtiment sur l’année. Un autre paramètre important à prendre en compte
lorsque une expérimentation est mise en place est son échelle. En effet, il n’est pas
...
d’expertise est ainsi nécessaire de définir le niveau de détail des modèles représentant
au mieux le système afin d’éviter des erreurs numériques ou des résultats erronés.


Il est aussi clair que les installations sont en réalité imparfaites et même dans
...
équipement peut avoir des caractéristiques différentes que ceux de sa fiche technique\dots
Il arrive aussi que un contrôleur puisse avoir des ratés où que le système sur chantier ne
...
réelles et peut être utilisé pour confronter les résultats théoriques à la réalité. Elle
est cependant couteuse et complexe à mettre en place. À l’inverse, la modélisation est
contrainte par des systèmes d’équations approchant la réalité mais est moins couteuse à
mettre en place et offre une liberté importante sur les conditions de fonctionnement.
L’ensemble des paramètres étant contrôlables, la modélisation offre en effet l’opportunité
d’évaluer différentes configurations. Elle est donc particulièrement adaptée pour des
études paramétrique nécessitant de faire varier chaque paramètres indépendamment les uns
des autres pour analyser leur effets respectifs. De plus les études paramétrique ou de
faisabilité, le dimensionnement, ou encore l’optimisation nécessite de nombreuses
itération sur des périodes importantes. L’expérimentation de par ces contraintes, ne
permet donc pas de répondre à la problématique et l’approche par modélisation est donc le
choix le plus pertinent.
Cette conclusion nécessite cependant d’être corrigé. Bien que
seule la simulation puisse répondre à ces questions, l’expérimentation est complémentaire
et permet de vérifier le modèle numérique. L’approche la plus correcte serait donc de
réaliser un modèle numérique pour ensuite le valider à travers une approche expérimentale.
Une fois validé, le modèle peut être utilisé pour étudier l’impact de diverses variations.
...
détaillé dans le chapitre suivant, les modèles retenues ont fait pour la plupart l’objet
de vérifications analytiques \parencite{Bontemps2016}. Bien que ne garantissant pas la validité
du modèle global, les tests et vérification effectuées au niveau du composant permettent d’apporter
une confiance dans les résultats du modèle solaire global.
...
contrôleur ou pompe.
L’$IEA$ l’utilise à travers la tâche $26$ \parencite{Task262003}


\itodo{Simulink Mosallat2013686}

\itodo{ISOFOTON SENSOLAR F-CHART Cuadros200796}
...
\begin{figure}
    \centering
    \itodo{Corriger température mini stockage à 40 pas 45}
