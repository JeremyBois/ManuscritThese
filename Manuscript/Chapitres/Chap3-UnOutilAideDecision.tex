...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics{Ressources/Images/Demonstration/xdat_complete.png}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics{Ressources/Images/Demonstration/xdat_curseur.png}
        \caption{}
...
L’utilisation de méta-modèle s’est fortement développée, et ceux pour des applications
diverses. Certains les utilises pour faire de l’optimisation, d’autres dans l’optique de
réaliser une analyse de sensibilité détaillée avec des méthode de décomposition de la
variance (voir Fig~\ref{fig:classement_methode_sensibilite}). \textcite{Armand-Decker2015}
les utilisent en substituts à un modèle de bâtiment réalisé sous \textit{Energy Plus} afin
de réduire le temps nécessaire pour l’optimisation multi- objectifs d’un bâtiment multi-
étage en bois. Plus récemment, \textcite{Faggianelli201761} a utilisé les méta-modèles
...
dans un temps très cours à un problème spécifique sans la complexité d’un logiciel de
modélisation.
...
\end{equation}
La construction d’un méta-modèle se traduit donc par la détermination des ensemble de
...
minimisation est réalisée par la méthode des moindres carrées et consiste à minimiser
...
\begin{figure}
    ...
    \caption[Description de la méthode d’aide à la décision retenue]
            {Description de la méthode d’aide à la décision retenue.}
...
La littérature décrit déjà les améliorations proposées à travers l’évaluation de la
performance de l’algorithme \abr{ABC} comme illustré à travers ce chapitre. Ainsi, cette
section n’a pas vocation a montrée le bénéfice apporté par les différentes améliorations
proposées mais il reste cependant nécessaire de montrer que l’algorithme fonctionne
correctement. Dans cette optique les résultats présentés dans cette section sont
sélectionnés afin de couvrir les principales difficultés existantes~:
\begin{itemize}
  ...
  \item Non-uniformité avec faible densité près de l’optimal~: DTLZ5 (3 objectifs), ZDT6
  \item Discontinue~: DTLZ6 (3 objectifs), Kursawe
\end{itemize}

\itodo{Hypervolume et distance générationnelle sur ZDT1, 2, 4, 6 et DTLZ5. DTLZ6.}
\itodo{Illustration avec Hanne1 Kursawe}
...
de sensibilité globale puis de réduire le temps d’évaluation du modèle grâce à des méta-
modèles. Bien que nécessaire de ce cas précis, il reste préférable de conserver le modèle
...
multi-objectifss est donc nécessaire. Le problème étant fortement combinatoire,
...
un algorithme de colonie d’abeilles virtuelles (\abr{ABC}) a été adaptée
aux problèmes multi-objectifss. Ce choix a été retenue principalement car l’approche
...
fait de retenir l’\abr{OBLM} (\defref{def:oblm}) afin d’offrir une meilleure
...
\begin{figure}
    \centering
    \includegraphics{Ressources/Images/chapitre3_bilan.pdf}
...
\textit{XDAT}.
La mise en place de cette méthodologie est décrite dans le dernier chapitre à travers l’étude
