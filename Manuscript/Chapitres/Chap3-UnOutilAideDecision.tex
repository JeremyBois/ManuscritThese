...
Dans la section précédente, chaque élément utilisé dans l’algorithme a été détaillé.
Les avantages et inconvénients des approches retenues ont aussi été explicité. Cette
section est la mise en commun de l’ensemble des améliorations retenues permettant
de construire le méta-heuristique MOABC utilisé dans ces travaux.

L’algorithme multi-objectif ABC retenue peux être définie par Fig.~\ref{fig:abc_complet}.
\begin{figure}
    \begin{center}
...
subit une variation en tenant compte d’une source de l’archive et d’une autre source.
Durant la phase des ouvrières (Algorithm~\ref{alg:onlooker_phase}), la qualité de chaque source \eqref{eq:attribution_prob_to_source}
est utilisée afin d’exploiter les plus prometteuses.
Finalement, si une source a subit le \emph{nombre maximum d’essai}
alors une exploratrice (Algorithm~\ref{alg:scout_phase}) réinitialise sa position.
Les phases des butineuses, des ouvrières, et des exploratrices sont ainsi répétées tant que
la condition d’arrêt n’est pas atteinte. L’archive est mis à jour après chaque phase
ne conservant que les meilleures solutions formant le front de Pareto.
...
\begin{algorithm}\label{alg:init_phase}
  ...
  \For{$i \leftarrow 0$ \KwTo \ANbrSources}
  {
    ...
    \For{$j \leftarrow 0$ \KwTo \ANbrCriteria}
    {
      ...
      avec $a_{j}$, $b_{j}$ respectivement les bornes inférieures et supérieures du critère
    }
...
\begin{algorithm}\label{alg:maj_phase}
  ...
  \For{$\ABee \in \ABees$}
  {
    Calcul du vecteur objectif pour l’\ABee ($\check{\vec{F}}$) et pour sa \ASource $i$ ($\vec{F}$),
    ...
    \For{$m \leftarrow 1$ \KwTo M}
    {
      ...
      \begin{aligned}
      \check{F}_{m} &= d_{m}(\check{\vec{x_{i}}}) + p_{m}(\check{\vec{x_{i}}}) \\
      F_{m}         &= d_{m}(\vec{x_{i}}) + p_{m}(\vec{x_{i}}) \\
      \end{aligned}
      \end{algomathdisplay}
    }
    \If{$\check{\vec{x_{i}}} \succ \vec{x_{i}}$}
    {
      \AComment{Mise à jour de la \ASource à partir de l’\ABee}
      $\ASource_{i} \leftarrow \ABee$\;
      \AComment{On réinitialise le nombre d’essais pour la \ASource $i$}
      $\ATrial_{i} \leftarrow 0$\;
    }
    \Else
    {
      \AComment{On incrémente le nombre d’essais pour la \ASource $i$}
      $\ATrial_{i} \pluseq 1$\;
    }
  }
...
\begin{algorithm}\label{alg:employed_phase}
  ...
  \For{$i \leftarrow 0$ \KwTo \ANbrSources}
  {
    ...
    Sélection aléatoire d’une source $p$ ($p \neq i$) dans la \APopulation\;
    \vspace{1em}  % Add some space between two blocs
    Génération d’une nouvelle position $\check{\vec{x_{i}}}$ à partir de la
    position $\vec{x_{i}}$ de la \ASource $i$ pour chaque \AButineuse $i$\;
    \If{$\ATirageA < \ARatio $ }
      {
      \For{$j \leftarrow 0$ \KwTo \ANbrCriteria}
      {
    ...
    \vspace{1em}  % Add some space between two blocs
    Évaluation des objectifs pour la nouvelle position $\check{\vec{x_{i}}}$\;
  ...
  \For{$\AOuvriere \in \AOnlookers$}
    {
      ...
      Sélection aléatoire d’une source $k$ ($k \neq i$) dans l’\AArchive\;
      Génération d’une nouvelle position $\check{\vec{x_{i}}}$ à partir de la
      position $\vec{x_{i}}$ de la \ASource $i$\;
      \For{$j \leftarrow 0$ \KwTo \ANbrCriteria}
      {
      \begin{algomathdisplay}
        \check{x_{ij}} =%
      ...
      Où \AMR est la probabilité de réaliser une modification (fixée à 0,5) et
      \ATirageB un nombre aléatoire entre 0 et 1\;
      \If{\AOuvriere respecte toutes les contraintes}
      {
        \AComment{Ajout de la solution trouvée par l’\AOuvriere à l’archive}
