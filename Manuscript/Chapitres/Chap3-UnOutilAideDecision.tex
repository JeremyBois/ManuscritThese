...
\textcite{Iooss2011} propose un diagramme de décision (Fig~\ref{fig:classement_methode})
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Principe des méta-modèles} % (fold)
\label{ssub:principe_des_meta_modeles}
Les modèles de simulation dynamique dans le bâtiment nécessitent un temps de simulation
important non compatible avec des méthodes nécessitant de nombreuses évaluations comme
l’optimisation. Afin de palier à ce problème, il existe plusieurs solutions comme les
méthodes de réduction de modèle ou l’utilisation de modèles de substitution. Cette partie
s’intéresse particulièrement au modèle de substitution qui permettent de conserver la
précision d’un modèle complexe tout en réduisant significativement le temps de simulation.
Le principe est d’approximer les sorties d’intérêts grâce à une fonction analytique appelé
méta-modèle. La complexité des interactions entre les variables d’entrées de la variable
de sortie d’intérêt est définie par le degré du méta-modèle.

\itodo{Littérature montrant l’attrait pour ces techniques}
L’utilisation de méta-modèle s’est fortement développé, et ceux pour des applications
diverses. Certain les utilises pour faire de l’optimisation, d’autre dans l’optique
...
garantie de performance pour un bâtiment de bureaux. L’approche a de plus été comparée
avec ...
\textcite{Armand-Decker2015} qui l’utilise pour
l’optimisation multi-objectif d’un bâtiment reprend de manière descriptive les étapes
nécessaire pour sa création.

Dans notre cas où le temps nécessaire pour une simulation est autour de \SI{1}{h},
l’utilisation de méta-modèle apporte un gain de temps significatif. Dans le cas d’une
optimisation, il est simple d’ajouter de nouveaux objectifs / contraintes et d’encapsuler
les fonctions d’un un logiciel sans dépendances avec les outils utilisés pour la création
des modèles. Il est alors possible de créer des outils métiers répondant à un problème
spécifique sans la contrainte de la complexité ou de l’apprentissage d’un nouvel outil.
C’est donc une approche polyvalente et adaptable. Malgré de nombreux avantages il est
important de garder à l’esprit que ces modèles de substitution ne sont valable que pour
l’espace des variables d’entrées utilisées pour leur création. Si un nouveau paramètre est
ajouté, ou si les bornes d’un paramètre existant est modifié, le méta-modèle doit être
recréé. Finalement la création d’un méta-modèle n’est pas toujours possible ou peut demander
un très grand nombre de simulation. Il est donc nécessaire de prendre en considération~:
\begin{itemize}
  \item Le nombre de simulations nécessaires pour créer le méta-modèle
  \item Le nombre de simulation nécessaires pour l’optimisation sans méta-modèle
  \item La diversité voulue dans l’ensemble de solutions dominées
...
\end{itemize}
% subsubsection principe_des_meta_modeles (end)


\subsubsection{Approche retenue} % (fold)
\label{ssub:approche_retenue}
~
\iunsure{Sert à rien de re décrire les équations et le détail des polynômes du chaos}
\iunsure{Juste citer Merheb et Decker qui blablate déjà beaucoup}
Il existe plusieurs moyens permettant de construire un méta-modèle. Dans ces travaux
l’implémentation réalisée par \textcite{Rania2013} avec la bibliothèque
\href{http://openturns.org/}{OpenTURNS} et à l’algorithme décrit par \textcite{Malen2009}
est retenue.

\itodo{Ajouter description de la méthode employée par Rania~: voir p.145 Decker}

\itodo{Ajouter ordre~: sensibilité --> méta modèle --> optimisation --> décision}
Comme il a été vu le méta-modèle est valable uniquement pour le jeu de variables
utilisé pour le construire. Dans cette optique la méthode de criblage de Morris
sera utilisée dans un premier temps afin d’éliminer les variables non influentes.
Dans un second temps un échantillon représentatif de l’espace des variables d’entrée
sera utilisé afin de construire le méta-modèle. Celui-ci sera ensuite vérifié avec
d’autres combinaisons de paramètre pour évaluer sa fidélité. De plus différentes tailles
d’échantillon seront utilisés afin de déterminer le nombre minimale de simulation nécessaires
Finalement les méta-modèles seront utilisé afin de réaliser l’optimisation multi-objectif.
En parallèle l’optimisation sera réalisée avec le modèle original afin d’évaluer le nombre de simulation
nécessaire pour converger vers les solutions optimales. Il sera ainsi possible à travers
la comparaison d’évaluer si la construction du méta-modèle apporte un gain de temps réel. Il
sera aussi possible de comparer les solutions trouvées avec les deux approches.
Les méta-modèles associés aux objectifs et contraintes étant propres au cas d’étude,
le détail de construction est décrit à travers le cas d’étude (\mtodo{Ref chap 4}).
...
décideur[s]. La recherche est alors fortement biaisée en amont, limitant
