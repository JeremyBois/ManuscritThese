...
étudiées sont sélectionnées sur la connaissance seule du système et des premiers résultats
...
des méthodes existantes en deux grande familles~: les méthodes de criblage et les méthodes
...
la moyenne (à cause des compensations dues au signes).
...
Le principe est illustré par \textcite{Iooss2011} à travers deux résultats obtenues
...
pour l’objectif de droite montrent eux que les facteurs $K_{s}$, $Z_{v}$, $Q$, $C_{b}$,
...
Récemment \mtodo{Anto}{Anto} a utilisé les méta-modèles dans le cadre de garantie de
...
(Fig~\mtodo{Ajouter ref fig}) est utilisée comme générateur pseudo aléatoire.
...
correspondant à ces préférences. En parallèle, l’optimisation sera réalisée avec le modèle
...
Ce terme permet de caractériser la forme d’un ensemble. Dans le cas de fronts continues,
...
\parencite{Bonabeau1999} défini à travers quatre propriétés illustrés à travers le
...
de la distribution de \textit{Lévy} retenue qui contrairement aux autres distributions n’a
d’expression analytique que pour certaines valeurs de paramètres (Fig.~\ref{fig:distribution_pdf}).
...
L’algorithme peut être définie par Fig.~\ref{fig:abc_complet} et le détail de chaque
