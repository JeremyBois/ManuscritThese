...
Upgrading Solutions) implémentent une méthode intéractive afin d’aider
...
solution finale à ses contraintes et ses préférences propres de manière intéractive.
...
conservant la nature multi- objectif du problème (\munsure{Ajouter citation}). Dans cette
...
\cite{Collette2002} afin d’obtenir plus d’informations.
...
\cite{Rivallain2013} l’utilise pour optimiser un bâtiment similaire à la
...
% ------------------------------------------------------------------------------
\subsection{Choix retenu} % (fold)
\label{sub:choix_retenu}
\itodo{Faire lien avec chapitre 2 pour temps long et cardinalité}
Dans cette section les différentes approches existantes ont été décrites. La non
connaissance de la solution optimale écarte les approches utilisant une recherche par
chemin optimal. La forte cardinalité du problème combinatoire écarte les approches
énumératives et par programmation dynamique car le nombre d’évaluation nécessaires serait
alors trop important (\mtodo{Ajouter nombre nécessaire}). La programmation dynamique est
aussi écarté Finalement, une méthode approchée par méta-heuristique est retenue car la
connaissance du cas d’étude ne permet pas de définir une heuristique spécialisée.
(Fig~\ref{fig:multi_meta}). Même si ces approches ne garantissent pas l’obtention de
l’ensemble des solutions optimales au sens de Pareto, les travaux précédents ont montré
\parencite{Rivallain2013,Recht2016}) que ces approches permettent de l’approximer
...
\itodo{Faire un bilan sur ce qu’il faut retenir de toutes ces variations}
...
\itodo{$d_{i}$ est un effet élémentaire. Clarifier dans le texte}
