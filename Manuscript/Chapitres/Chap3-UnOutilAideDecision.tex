...
L’utilisation d’une méthode multi-critère d’aide à la décision ($MCDA$, Multi-Criteria
...
des logiciels de simulation dynamique (\textit{STD}, \textit{CFD}\dots) comme objectifs.
...
des $MCDA$ ainsi que de nombreuses références sont proposées par \textcite{BenMena2000}.
...
\href{http://www.xdat.org/}{\textit{XDAT}} sera utilisé afin de permettre au décideur d’adapter la
...
\label{par:principe}
La méthode de Morris utilise un plan $OAT$ (One (Factor) At the Time) et est basée
sur l’$EEM$ (Elementary Effect Method) \parencite{Saltelli2004}. Chaque facteur
...
(répétitions) à l’intérieur du plan $OAT$ (\figref{fig:fonctionnement_morris}).
...
\iunsure{Ajouter graphique expliquant linéaire et non linéaire}
% Plus $\sigma$ est important par rapport à $\mu$ plus l’hypothèse de non linéarité
...
proposée est cependant faite par \textit{Brute force} et demande donc une puissance de
calcul importante.
Afin de palier à ce problème, \textcite{Ruano2012103} proposent une approximation de
la méthode par \textit{Brute force} pour un temps de calcul très court permettant d’obtenir
...
\textit{méthode de Morris} appliquée sur chaque objectif et contrainte. Le problème
...
\textit{voisinage} et faible s’il est seulement non-dominé dans ce voisinage (plusieurs
...
colonie d’abeilles virtuelles ($ABC$, Artificial Bee colony) requiert la
...
logiciel \textit{Dymola} par l’intermédiaire de son langage de script. De plus, un outil a
été développé sur la base de la bibliothèque \textit{BuildingsPy} afin de réaliser l’étude
paramétrique. Cet outil permet de simuler de manière concurrente le modèle, et ainsi
fortement réduire le temps nécessaire par simulation. Afin de profiter de ces outils et
simplifier le couplage, seules les solutions développées en \textit{Python} sont retenues.
Les suites logiciels telles que \href{http://jmetal.sourceforge.net/index.html}{jMetal}
ou \href{http://moeaframework.org/index.html}{MOEA Framework} sont ainsi écartées.
...
l’algorithme $ABC$ doivent être re-implémentées.
...
Comme de nombreuses méta-heuristiques, l’$ABC$ est inspiré du comportement des êtres
...
Algorithm (\textit{BA}), Bee Colony Optimization (\textit{BCO})\dots), la formulation de
l’$ABC$ reste la plus répandue et sera retenue dans ces travaux (\figref{fig:abc_repartition}).
...
L’algorithme $ABC$ est une approche itérative constituée de 3 phases principales.
...
L’\algref{alg:ABC_phases} décrit le principe de l’algorithme $ABC$ original.
...
évolutionnaires \parencite{Bi2011174,Zhao2010558} et d’autres s’inspirent du \textit{PSO}.
...
\textcite{Li2012320} mimiquent la formulation du \textit{PSO} en ajoutant une inertie, l’accélération et la prise
...
\textcite{Sharma2012213}, utilisent le vol de Lévy afin de réaliser une recherche locale
...
reproduction des \href{https://fr.wikipedia.org/wiki/Cuculidae}{cuculidés}, il a aussi
...
l’effet d’île caractéristique du vol de Lévy (\figref{fig:levy_vs_gaussian})
...
($OBL$ pour Opposite Based Learning) (\defref{def:oblm}).
...
approches utilisent une grille similaire (\textit{PAES}) \parencite{Knowles2000149},
...
$OBL$ (\defref{def:oblm}) afin d’obtenir une meilleure représentation du
