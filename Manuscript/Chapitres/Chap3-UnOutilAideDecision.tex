...
ELECTRE ou encore PROMETHEE font partis des principales méthodes développées
(\mtodo{Ajouter citation}) et la sélection d’une approche est lié au type de
problématique. Dans une problématique de choix, la méthode ELECTRE I
ou IS pourra être sélectionnée. Dans une problématique de rangement, les méthodes
ELECTRE II, III, ou IV pourront être utilisées.
...
L’approche la plus courante est l’utilisation de l’A* ou d’une de ces dérivées.
...
Le PSO (Particule Swarm Optimization), un autre algorithme, s’inspire du
...
existent pour automatiser ce paramétrage. \cite{Lopez-Ibanez2012861} a ainsi
...
Un modèle mathématique est décrit par \cite{Camazine1991547} permet de simuler
ce comportement social caractéristique.
...
La formulation de l’ABC reste la plus répandue et sera retenue dans ces travaux
...
\begin{equation}\label{eq:update_source}
  \check{x_{ij}} = x_{ij} + RandUniform(-1, 1) \times (x_{ij} - x_{aj}) \\
\end{equation}
...
d’optimisation binaire \cite{Kashan2012342}, et combinatoires \cite{Karaboga20113021}.
...
\cite{Zhu20103166} développe l’algorithme GBest Artificial Bee Colony (\textit{GABC}) en
...
initialise la position des sources en utilisant une séquence issue de la théorie
...
approche comprenant un unique essaim est proposé par \cite{Akbari201239} et le nomme
...
aléatoirement.
\cite{Omkar2011489} considère un essaim par objectif et nomme son approche : \textit{VEABC}
...
de Crowding, l’auteur ajoute une coopération entre les essaims pour la sélection des
...
\begin{equation}
  x(t + 1) = x(t) + \omega \times longueurPas \times \Phi
\end{equation}
avec $\omega$ un facteur d’échelle permettant de limiter l’agressivité du pas,
$longueurPas$ un nombre tiré aléatoirement dans la distribution de Lévy et $\Phi$
...
\cite{Rahnamayan2008906} l’adapte pour l’algorithme Differential Evolution (\textit{DE})
...
est retenue.
\cite{Laumanns2002263} propose une autre approche, séparer l’espace des solutions en une
