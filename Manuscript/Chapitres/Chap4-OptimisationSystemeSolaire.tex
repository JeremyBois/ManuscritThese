...
est réalisée afin de mieux comprendre les intéractions principales existantes~:
\begin{itemize}
...
\itodo{Décrire paramètres algo et nbr itérations}
\ftodo{Pareto front pour montrer les solutions obtenues}
...
toujours une bonne recommendation mais la \figref{fig:conso_ref_vs_diffconso} permet
...
d’évaluer le coût réel. Par exemple, il est souvent considéré pour l’isolant un coût fixe
dépendant uniquement de son épaisseur. Cette approximation bien que valide pour une certaine
plage de variation, oublie de considérée le bâtiment dans son ensemble. Ainsi en fonction
de la structure du bâtiment des surcoût \emph{importants} peuvent survenir lorsque on passe
d’une épaisseur à une autre. De plus si l’objectif est de calculer un temps de retour
...
% conso_chauffage_joinplot
    ...
    \caption[Corrélation entre la qualité de l’enveloppe et la performance du \abr{SSC}]
             {Corrélations entre la qualité de l’enveloppe et la performance du \abr{SSC}
              en fonction des indicateurs suivants~: (a) $Conso_{app}$, (b) $F_{sav, ext}$,
              (c) $Conso_{ref} - Conso_{app}$.}
...
Dans le processus d’aide à la décision retenue, il est nécessaire dans un premier
temps de faire une optimisation multi-objectifs. Le front obtenu est ensuite
évaluer de manière intéractive grâce à une méthode paramétrique. Cependant,
l’approche comme toutes approches impliquant une optimisation contraint à retenir
...
Afin de pouvoir analyser l’influence du type de capteur sur la performance du système,
il est cependant nécessaire de chercher à évaluer la performance maximale pour chaque
capteurs.
...
peu important. Dans le premier cas, il est logique de favoriser ce type de capteur,
...
% count_collector_type
    ...
    \caption[Occurences de chaque type de capteurs pour les solutions du front de Pareto]
             {Occurences de chaque type de capteurs pour les solutions du front de Pareto
...
l’écart de performance est peu impactant. Par contre pour les deux autres capteurs,
l’écart de performance est plus important ($Conso_{app}$ variant de \SIrange{100}{200}{kWh}).
