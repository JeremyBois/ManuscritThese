...
compromis entre qualité de l’enveloppe et performance du \abr{SSC} sur le chauffage, est
aussi identifiée.



Le choix des objectifs $F_{sav}^{ECS}$ et $F_{sav}^{CH}$ pour
...
facteur impactant pour Bordeaux alors que sur Strasbourg $\Delta min_{capteur}$ est aussi
un facteur influent. En effet, les besoins en chauffage étant faibles relativement aux
besoins pour la production d’\abr{ECS}, la consommation totale ($Conso_{app}$) est
uniquement influencée par $\Delta min_{tampon}$ sur Bordeaux. La même observation peut
être faite pour le ballon tampon dont le volume impacte uniquement la $Conso_{app}^{CH}$
sur Bordeaux. Enfin les caractéristiques des capteurs sont influentes à la fois sur la
...
(degré du polynôme) est défini par la zone grise associée à l’axe vertical de droite.
L’axe de gauche est lui utilisé comme échelle de valeurs pour la courbe (\abr{RMSE} ou \abr{MAE}).
...
En effet, à partir d’un échantillon de taille $100$, le méta-modèle est d’ordre $3$ et précis à
...
Les résultats montrent aussi que le nombre de capteurs \abr{PV} est inversement
...
de réduire de $5$ (\SI{8.2}{m^{2}}) le nombre de capteurs \abr{PV} (\figref{fig:fsav_pv_bor}). De
plus, la production des capteurs solaires thermiques est en adéquation
avec la demande alors que le bilan \textit{production / consommation} est réalisé à l’échelle annuelle
...
Comme explicité lors du choix des collecteurs, les capteurs plans récents
ont tous un rendement optique ($\eta_{0}$) important et de ce fait, le coefficient $a_{1}$ est
le principal responsable des différences de performance.

De plus, le capteur \textit{SkyPro}, qui a pourtant le rendement optique ($\eta_{0}$) le moins élevé,
permet au \abr{SSC} d’obtenir le meilleur $F_{sav,\, ext}$. En effet, la surface considérée dans le calcul du
...
\begin{blockdescription}{Solution 1 (\abr{S1})~:}
  ...
  \item[Solution 4 (\abr{S4})~:] Cette solution correspond à la solution maximisant le $F_{sav,\,ext}$ lorsque
                                 ...
                                 de $8$ capteurs \abr{PV} supplémentaires. Enfin
