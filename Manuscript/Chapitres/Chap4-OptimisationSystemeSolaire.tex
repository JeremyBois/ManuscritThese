...
retenue pour la consigne de chauffage ($19$-$18$-\SI{16}{\celsius}) comme pour la ventilation
($90$-\SI{20}{m^{3}\per h}). Pour le puisage en $ECS$, le profil \textbf{Réaliste} est retenue tout en tenant
...
consommation totale sur les quatre usages est considérés. Le \abr{SSC} couvrant en totalité
...
nombre maximal de capteur pouvant loger sur chaque pan de toiture. Malgré les différences géométriques
...
être noté que les capteurs sous-vides (\textit{CPC $14$ Star}, \textit{SKY PRO $12$ CPC$58$})
sont beaucoup moins sensible à la variation de l’angle d’incidence expliquant que
...
détaillés \ref{ssub:deperditions_a_travers_les_fenetres}. Le type de vitrage est appliqué
...
existe~: l’optimisation risque de converger vers un ensemble de solution très réduit voir
une solution unique.
...
d’obtenir une bonne approximation (\figref{fig:mae_rmse_qualite_ref}).
...
$3$~: le modèle substituant le calcul de la $Conso_{ref}^{CH}$ sur Strasbourg. Un écart de
...
Finalement l’échantillon met déjà en avant l’interêt d’un \abr{SSC}, en particulier sur
