...
ne pas sur-estimer la performance\abr{SSC}.
...
durant la période estival (\figref{fig:diff_ensoleillement_bor_stras}). Enfin,
...
de référence pour chaque niveau d’enveloppe afin d’évaluer uniquement
l’amélioration induite par le \abr{SSC} et non l’amélioration apportée par l’amélioration
de l’enveloppe. Un modèle de référence sans apports solaires
(tous électrique) est donc modélisé. Il admet un unique ballon de \SI{200}{\litre} pour le stockage
...
dominées permettant d’obtenir une \abr{MEPOS} solaire.
...
\begin{figure}
    ...
    \caption[Graphe d’influence\abr{SSCpour }Bordeaux et Strasbourg]
            {Graphe d’influence\abr{SSCpour }Bordeaux (gauche) et Strasbourg (droite) où les
...
\abr{RMSE} et \abr{MAE} sont maximisées.

Le \tabref{tab:meta_vs_optimisation} montre que l’erreur moyenne quadratique augmente
...
l’impact de la taille de l’échantillon (annexe, \figref{fig:rmse_mae}). Il avait en
...
\begin{figure}
    \centering
    \includegraphics{Ressources/Images/MetaModele/meta_optimization.pdf}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics{Ressources/Images/EtudeDeCas/Bordeaux_algo.pdf}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics{Ressources/Images/EtudeDeCas/Strasbourg_algo.pdf}
        \caption{}
...
nombre de capteurs \abr{PV} en fonction des autres objectifs.

\paragraph{} % (fold)
Les solutions devant toutes respecter un bilan positif, il existe un équilibre en
production solaire par les capteurs solaires thermiques et par les capteurs \abr{PV}.
...
Améliorer au delà la performance du système sur le chauffage, ou sur la production d’\abr{ECS},
ne permet pas de réduire le nombre de capteurs \abr{PV}. La consommation de l’appoint
est en effet déjà très faible et le production des capteurs \abr{PV} est principalement
...
\SI{84}{\percent} sur la production d’\abr{ECS}, ne permet pas de réduire le nombre de capteurs \abr{PV} nécessaires.
Comme pour Bordeaux, le gain absolue sur la consommation n’est pas suffisant pour permettre de se passer d’un
autre capteur \abr{PV}.
La production des capteurs \abr{PV} admet en effet des sauts discrets car il a été retenue de faire
...
La répartition des capteurs \abr{PV} est en effet dépendante du nombre de capteurs
solaires thermiques installés. Lorsque que le nombre réduit, il est possible de loger plus
de capteurs \abr{PV} sur le pan sud et donc d’améliorer la production.

Afin d’apprécier l’étendu du front de Pareto, il est aussi renseigné les bornes de
l’espace des objectifs (\tabref{tab:bornes_front_pareto}) correspondant aux points nadir
...
\end{itemize}
Sur Bordeaux, le \abr{SSC} permet au maximum de réduire de
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Répartition des critères de décisions} % (fold)
\label{ssub:repartition_des_criteres_de_decisions}
\itodo{Variation algo en fonction objectif à maximiser. faire avec XDAT}
\ttodo{Décrire critère pour les solutions maxisant chaque taux}
...
la production d’\abr{ECS}, ni les meilleures sur le chauffage. Il apparaît donc
qu’il existe un compromis entre amélioration de la performance sur le chauffage et
amélioration de la production d’\abr{ECS}. En effet en cherchant à maximiser par exemple la production
d’\abr{ECS} on réduit la part apportée au chauffage et inversement.

\ftodo{Countplot pour répartition des critères}
% subsubsection repartition_des_criteres_de_decisions (end)
% subsection analyse_globale_des_solutions_non_dominees (end)
...
autant sur le chauffage ou la production d’\abr{ECS}. De plus la variation de la
...
plus la part à couvrir par les capteurs \abr{PV} diminue.
Cependant, comme expliqué au paragraphe précédent, la consommation de référence est plus
importante sur Strasbourg là où le \abr{SSC} est moins performant. Ainsi, le nombre
minimal de capteurs \abr{PV} est plus important sur Strasbourg.
...
thermiques de $5$ permet de réduire de $8$ le nombre de capteurs \abr{PV} nécessaire
(\figref{fig:fsav_pv_stras}). Sur Bordeaux une variation de $3$ capteurs thermique permet
de réduire de $5$ le nombre de capteurs \abr{PV} (\figref{fig:fsav_pv_bor}). Il est aussi
mis en évidence que les solutions encadrées sont celles qui maximisent le nombre de
capteurs \abr{PV} sur la toiture. Contrairement aux solutions en dehors du cadre, elles ne
...
même temps la surface minimale de capteurs \abr{PV}.
...
\itodo{Ajouter partie sur surface des capteurs}
\ftodo{Pair plot avec couleur == nombre de capteur TH}
Le processus d’aide à la décision retenue implique dans un premier temps, la réalisation
d’une optimisation multi-objectifs, puis grâce à une approche paramétrique le front est
réduit aux solutions de manière interactive afin de répondre aux critères du ou des
...
intéressantes. Ainsi afin de pouvoir analyser l’influence du type de capteur sur la
performance du système, il est nécessaire de chercher à évaluer la performance maximale
pour chaque capteurs suivant un processus d’optimisation indépendant.
Profitant d’un temps d’évaluation très faible grâce aux modèles de substitutions,
une approche itérative exploratoire est donc retenue.

En effet, l’optimisation a été réalisée dans un premier temps en considérant l’ensemble
des paramètres et montre une majorité écrasante de solutions qui considèrent un capteur
...
% count_collector_type
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics{Ressources/Images/EtudeDeCas/occurence_type_capteur_bor.pdf}
        \caption{}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics{Ressources/Images/EtudeDeCas/occurence_type_capteur_stras.pdf}
        \caption{}
...
Comme observée précédemment, le meilleur taux d’économie est obtenue par un compromis
entre l’économie sur le chauffage et sur l’\abr{ECS}. La production d’\abr{ECS} étant
plus importante, elle est favorisée. De plus peut importe le type de capteur, la
...
fiche technique des capteurs. En effet le capteur \textit{SkyPro} permet d’obtenir
la meilleure performance car le rendement est inférieure mais considère une surface
...
% type_collector_evolution
    \centering
    \includegraphics{Ressources/Images/EtudeDeCas/capteurs_type_perf_tampon.pdf}
    \includegraphics{Ressources/Images/EtudeDeCas/capteurs_type_perf_ecs.pdf}
...
permettre d’identifier chaque point pour chaque groupe de solutions formées en fonction du
...
Afin d’améliorer la fluidité de la lecture, un volume allant de \SIrange{100}{200}{\litre} sera considéré
comme \emph{faible ou petit} alors que un volume supérieur à \SI{300}{\litre} sera considéré comme \emph{important
ou gros}.

Indépendamment du nombre ou du type de capteurs solaires thermiques, il existe des solutions
optimales pour toutes les tailles de ballon tampon. De plus un $F_{sav,\, ext}$ important
est obtenu pour des petits comme des gros volumes de ballon tampon. Il semble donc possible
de retenir un volume de ballon tampon faible sans dégrader la performance globale du \abr{SSC}.
Par contre un volume de ballon \abr{ECS} important permet d’obtenir la meilleure
performance globale. Si on considère moins de $5$ capteurs par contre le volume
du ballon d’\abr{ECS} est moins impactant. Il est aussi noté que même si des volumes
importants sont favorisés, il existe des solutions très performantes avec des
volumes peu important d’\abr{ECS}. Ces solutions considèrent cependant un volume
de ballon tampon plus important. Il semble donc qu’il soit possible de favoriser
soit le chauffage, soit la production d’\abr{ECS} pour obtenir un taux d’économie
important. Ces résultats confirment les observations précédentes, un compromis est
nécessaire entre performance sur le chauffage et sur la production d’\abr{ECS}.

Pour chaque nombre de capteurs installés, l’écart entre la valeur minimale et
maximale augmente. Ainsi lorsque uniquement $2$
capteurs sont installés, $F_{sav,\, ext}$ varie de \SIrange{38}{46}{\percent} alors
que pour $7$ capteurs la variation est de \SIrange{57}{74}{\percent} soit une augmentation
de l’écart de \SI{9}{\percent}. En effet à mesure que le nombre de capteurs augmente,
la différence de performance entre les capteurs est accentuée. Ainsi l’écart entre
la performance maximale pour le capteur \textit{SkyPro} et le capteur \textit{EnergyEco}
est de \SI{5}{\percent} lorsque $7$ capteurs sont installées contre \SI{2.5}{\percent}
pour seulement $2$.
...
Au regard des résultats, il est donc intéressant de chercher à évaluer plus en détail l’impact
du volume des ballons sur la performance du \abr{SSC}. Cette analyse est donc réalisée
...
pour le ballon d’\abr{ECS} est plus souvent retenue sur Strasbourg. Le volume du ballon
tampon est plus variant. À l’inverse sur Bordeaux c’est un ballon tampon important qui est
majoritairement retenu alors que la taille du ballon \abr{ECS} est plus variable même si
un volume supérieur à \SI{300}{\litre} est majoritairement retenue.
Un volume important pour le ballon tampon permet d’améliorer le $F_{sav}^{CH}$ mais
n’impacte que faiblement le $F_{sav,\, ext}$. Les consommations nécessaires pour couvrir
les besoins en chauffage sont en effet faibles, ainsi améliorer l’efficacité du \abr{SSC}
sur le chauffage n’apporte pas de gain important. Il semble donc pertinent sur Bordeaux de
considérer un volume de tampon inférieur à \SI{100}{\litre} voir de chercher à améliorer
l’isolation et uniquement considérer un système solaire pour la production d’\abr{ECS}
avec $3$ ou $4$ collecteurs (annexe, \figref{fig:tanks_variations_bordeaux}). En effet
la $Conso_{ref}$ varient seulement de \SIrange{228}{620}{kWh} et les solutions optimales
observée montre que la $Conso_{app}$ elle varie de \SI{48}{470}{kWh}. Ainsi le reste
de cette section s’intéresse uniquement aux variations observée pour le climat de
...
% count_collector_type
    \centering
    \includegraphics{Ressources/Images/EtudeDeCas/tanks_count.pdf}
...
(\figref{fig:tanks_variations_locales_strasbourg}, annexe \figref{fig:tanks_variations_globales_strasbourg}).
...
En limitant la variation du volume des ballons (\SIrange{100}{200}{\litre}) il est possible d’obtenir respectivement
...
d’énergie solaire, la température d’un ballon tampon reste trop basse pour pouvoir être
ensuite valorisée. Finalement, il est aussi observé que même pour une plage de variation
réduite, le
\abr{SSC} est toujours capable de proposer des solutions maximisant soit le $F_{sav}^{CH}$,
soit le $F_{sav}^{ECS}$. Il est en effet observée une large variabilité dans les valeurs
...
fait des fortes interactions entre l’algorithme et les composants du \abr{SSC}, il n’est
...
% volume_collector_factorplot
    \centering
    \itodo{Refaire avec légende correcte (manque nom) pareil dans les annexes.}
    \includegraphics{Ressources/Images/EtudeDeCas/strasbourg_tanks_variations_local.pdf}
    \caption[Évolution de la performance du \abr{SSC} par secteur en fonction du volume des ballons sur Strasbourg.]
...
performance de l’enveloppe et la performance du \abr{SSC}. Dans cette optique, $3$
...
performance du \abr{SSC} et économie sur l’appoint.
...
\itodo{Reformuler}
Ainsi, le choix du niveau d’isolation doit bien être déterminé parallèlement au choix des
...
$100$ ans lorsque un \abr{SSC} est évaluée.
...
% conso_chauffage_joinplot
    \centering
    \itodo{Refaire avec légende mieux visible}
...
\label{sub:bilan}
\itodo{Faire bilan de ce qui a été observé}
...
% ..............................................................................
% ..............................................................................
\section{Aide à la décision a posteriori} % (fold)
\label{sec:aide_a_la_decision_a_posteriori}
...
Le processus d’aide à la décision a posteriori nécessite un ensemble de solutions
optimales. Durant ce processus les objectifs peuvent soit être similaires à ceux de
l’optimisation, soit ajouter des contraintes supplémentaires. En effet l’avantage d’une
optimisation multi-objectifs réside dans sa capacité à explorer l’espace des objectifs
afin de proposer les différentes combinaisons existantes sans introduire en amont de
préférence. De cette manière, de nouvelles connaissances sur le problème peuvent émerger
comme ce fut le cas dans ces travaux. Une fois la population obtenue, l’outil d’analyse
par coordonnées paramétrique \textit{XDAT} est utilisé pour réduire le nombre
de solutions. Cette partie illustre l’utilisation du logiciel pour le climat de
Strasbourg en considérant le front commun formé par les optimisations successives.


L’objectif de ces travaux était de mieux comprendre les interactions existantes entre le
système solaires combiné développé, et une maison à énergie positive mais aussi de
caractériser la performance du \abr{SSC}. Une approche exploratoire des différents
objectifs liés à la performance énergétique a semblé et s’est montrée être pertinente pour
mieux comprendre les interactions existantes. Cependant aucunes méthodes n’est parfaite et
l’approche retenue introduit un élitisme important sur certains critères de décisions. Il
a donc été proposé de mettre en place un processus itératif d’aide à la décision. Dans un
premier temps l’optimisation est réalisée en considérant l’ensemble des critères de
décisions pour les objectifs principaux. Ensuite en fonction des objectifs secondaires et
des résultats de la première optimisation, certains critères de décisions peuvent être
fixés ou la plage de variation réduite. Le processus d’optimisation est alors relancé et
les solutions optimales ajoutées dans une archive commune. Cette méthode a permis
d’identifier des compromis similaires aux solutions optimales mais proposant de nouvelles
combinaisons sur les critères de décisions. Ce processus permet ainsi d’affiner la
recherche afin de proposer un ensemble de solutions optimales à la fois par rapports aux
objectifs mais aussi aux contraintes propres du décideurs. Elle reste aussi adaptée pour
d’autres applications où le nombre d’objectif considérés est déjà important. En effet la
complexité augmente rapidement avec le nombre d’objectifs considérés et la littérature
montre que les l’efficacité des méta-heuristiques diminue à mesure que le nombre
d’objectifs augmente.
...
% section aide_a_la_decision_a_posteriori (end)
...
% ..............................................................................
% ..............................................................................
\section{Bilan et conclusions} % (fold)
\label{sec:bilan_et_conclusions}

% section bilan_et_conclusions (end)
