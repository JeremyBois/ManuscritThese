...
parmi en catalogue de solutions comme c’est déjà le cas pour la géométrie du bâtiment.
...
\paragraph{} % (fold)
Dans cette dernière partie, la sélection des solutions optimales par application de la préférence de l’utilisateur est illustré à travers
\figref{fig:xdat_exemple} où chaque ajout de préférence fabrique un nouveau cluster identifiable par une couleur spécifique
Les solutions grises respectent aucunes des contraintes et les autres couleurs répondent à un certain nombre
des préférences comme explicité ci-après.
Considérons le cas où le client souhaite minimiser la place prise par l’installation dans sa
maison. En réduisant l’intervalle autorisée pour le volume des ballons à l’aide de leur curseurs respectifs,
le front peut être réduit aux solution ayant un volume inférieur à \SI{200}{\litre} (courbes bleues).
Maintenant, il souhaite faire des économies sur l’isolation tout en limitant l’augmentation
de la consommation électrique. Dans ce cas il est nécessaire de faire varier les curseurs
sur les paramètres caractérisant l’enveloppe comme par exemple la résistance thermique des
murs, puis, d’observer la performance des solutions restantes (courbes roses).
À ce stage, l’ensemble des solutions nécessitent une consommation électrique de \SIrange{14}{17}{kWh\per\m^{2}\per an} et
une variation d’économie de \SI{300}{kWh \per an} est observée.
Il est aussi noté que l’espace de décision est toujours diverse en particulier sur la qualité
de l’enveloppe. Plusieurs choix sont ainsi possibles~:
\begin{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth, clip=true, trim=0mm 20mm 0mm 0mm]{Ressources/Images/EtudeDeCas/AideDecision/base.png}
    \\
    \includegraphics[width=\textwidth, clip=true, trim=0mm 20mm 0mm 0mm]{Ressources/Images/EtudeDeCas/AideDecision/volume_ballons.png}
    \\
    \includegraphics[width=\textwidth, clip=true, trim=0mm 20mm 0mm 0mm]{Ressources/Images/EtudeDeCas/AideDecision/final.png}
    \caption[Évolution de l’aide à la décision en fonction de choix fictif d’un décideur]
             {Évolution de l’aide à la décision en fonction de choix fictif d’un décideur. Les courbes grisées ne répondent à
             aucunes contraintes du décideur. Chaque couleur indique par contre d’une ou plusieurs contraintes.}
...
\begin{blockdescription}{Solution 1 (\abr{S1})~:}
  ...
  \item[Solution 3 (\abr{S3})~:] Cette solution maximise le $F_{sav}^{CH}$. $F_{sav}^{ECS}$ reste
                                 ...
                                 atteint car le gain sur le chauffage est nul et la perte de
  ...
  \item[Solution 5 (\abr{S5})~:] Cette solution maximise le $F_{sav,\,ext}$ lorsque le volume de chaque
                                 ballon est inférieur à \SI{250}{\litre}. Mis en exergue dans \ref{sub:influence_du_volume_des_ballons},
...
\abr{SSC} appliqués aux maisons à énergie positive.
...
des critères de décisions. Ensuite en fonction des résultats de la première optimisation,
...
adaptée aux secteur du bâtiment afin que chaque acteurs puissent participer aux
décisions. De plus l’ajout d’un objectif ou d’une contraintes étant trivial, elle
permet aussi de tenir compte des contraintes de chaque corps de métiers.
...
d’obtenir la meilleure économie. Il est possible d’obtenir une performance équivalente
pour différentes tailles de ballons. En plus de libérer une place conséquente,
...
Il est aussi identifié l’existence d’un compromis entre qualité de l’enveloppe et
...
Concernant les capteurs, le choix du type ne n’est que peu influent tant que le capteur
est récent. De plus au delà de $4$ capteurs sur Bordeaux et de $6$ sur Strasbourg le
