...
% ------------------------------------------------------------------------------
\subsubsection{Variation du nombre de capteurs \abr{PV}} % (fold)
...
\end{figure}
% subsubsection variation_du_nombre_de_capteurs_pv (end)
...
% ------------------------------------------------------------------------------
\subsection{Influence du type de capteur} % (fold)
...
% subsection influence_du_type_de_capteur (end)
...
en annexe () où le volume exact est décrit par la couleur du point. Les résultats montrent
qu’un volume minimal sur le tampon (\SI{100}{\litre}) et un volume de \SIrange{150}{200}{\litre}
pour le ballon d’\abr{ECS} est suffisant sur Strasbourg.

Il est aussi noté que lorsque le nombre de capteurs est faible, il est plus intéressant de considérer un volume faible
pour les deux ballons. En effet, si il n’y a pas assez d’énergie solaire, la température
d’un ballon tampon reste trop basse pour pouvoir être ensuite valorisée.
Finalement, il est aussi observé que même pour une plage de variation réduite, le
\abr{SSC} est toujours capable de proposer des solutions maximisant soit le $F_{sav}^{CH}$,
soit le $F_{sav}^{ECS}$. Il semble donc que les critères de décisions sur l’algorithme
soient les principaux responsables de l’orientation du \abr{SSC}.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
Il a aussi été mis en évidence que le \abr{SSC} est toujours capable de proposer
la même plage de performance pour le chauffage et la production d’\abr{ECS}.
La dernière partie de cette section s’intéresse donc à l’influence des critères
de décisions propre à l’algorithme.
% subsection influence_du_volume_des_ballons (end)



% ------------------------------------------------------------------------------
\subsection{Influence de l’algorithme de contrôle} % (fold)
\label{sub:influence_de_l_algorithme_de_controle}
\itodo{Ajouter description sur tendances de l’algorithme}

% subsection influence_de_l_algorithme_de_controle (end)
