...
du \abr{SSC} mais aussi d’étudier les interactions existantes entre enveloppe du bâtiment et le \abr{SSC}.
...
durant la période estivale (\figref{fig:diff_ensoleillement_bor_stras}). Ce comportement
...
d’autre part les capteurs photovoltaïques (\abr{PV}). D’après la littérature et les résultats
...
Avec $Surf_{PV}^{tot}$ la surface totale de capteur \abr{PV} installée,
...
pas la performance du \abr{SSC}. Aussi, les charges internes propres
...
en amont des simulations du \abr{SSC}.

En couplant les résultats des deux simulations, la consommation totale sur les quatre
usages est considérée. Le \abr{SSC} couvrant en totalité les besoins sur la période non
simulée, la consommation de l’appoint est équivalente à la consommation des pompes. Il est
donc possible d’approximer la $Conso_{app}$ annuelle par la consommation de l’appoint et
des pompes sur la période simulée. La consommation totale est alors obtenu par
\eqref{eq:conso_totale}. Il est important de noter que la $Conso_{app}$ est calculée à
chaque pas de temps. Elle tient donc compte implicitement de l’adéquation entre besoins
(chauffage et \abr{ECS}) et de la production solaire thermique ($Prod_{sol}^{valorisée}$).
À l’inverse, la $Prod_{PV}$ est elle pré-calculée et ne tient donc pas compte de
l’adéquation avec les besoins (électroménager et éclairage).
...
\begin{tabular}{l C{1cm} C{1.5cm} l r}
  ...
  \addlinespace
  Volume ballon tampon                         & \num{100}     & \num{500}   & \multirow{2}{*}{Géométrie adaptée proportionnellement} & \multirow{2}{*}{\si{\litre}}             \\
  Volume ballon $ECS$                          & \num{100}     & \num{500}   &                                                                      \\
  $Isolant_{ballon,\, ep}$ tampon              & \num{0.055}   & \num{0.12}  &  \multirow{2}{*}{Résistance dépendante du volume du ballon} & \multirow{2}{*}{\si{\metre}}   \\
  $Isolant_{ballon,\, ep}$ \abr{ECS}           & \num{0.055}   & \num{0.12}  &                                                           \\
  $Isolant_{réseau,\, ep}$                     & \num{0.013}   & \num{0.04}  & Résistance dépendante du nombre de capteurs  & \si{\metre}                        \\
  $\Delta T_{sol}$                             & \num{5}       & \num{20}    &  - & \multirow{4}{*}{\si{\degree}}      \\
  $\Delta T min_{capteur}$                     & \num{0}       & \num{30}    &  -                                                                   \\
  $\Delta T min_{tampon}$                      & \num{0}       & \num{30}    &  -                                                                   \\
  $T3_{min}$                                   & \num{15}      & \num{40}    &  -                                                                   \\
  \\
  \addlinespace[\defaultaddspace]
  \multicolumn{5}{l}{\textbf{Enveloppe du bâtiment}}                                                                              \\
  \midrule
  Type de vitrage                              & -             & -           &  Voir \tabref{tab:carac_vitrages} &  -                                        \\
  $R$ murs                                     & \num{4}       & \num{7}     &  - & \multirow{3}{*}{\si{(m^{2}\period\kelvin)/W}}                                                                 \\
  $R$ plafond                                  & \num{6}       & \num{10}    &  -                                                                   \\
  $R$ plancher                                 & \num{6}       & \num{10}    &  -                                                                   \\
  Surface vitrée est                           & \num{4.3}     & \num{6.46}  & \multirow{4}{*}{Surface totale du mur \SI{26.4}{\metre\squared}} & \multirow{4}{*}{\si{\metre\squared}}            \\
  Surface vitrée nord                          & \num{0.46}    & \num{0.684} &                                                                      \\
  Surface vitrée sud                           & \num{5.42}    & \num{8.13}  &                                                                      \\
...
est aussi contrôlée par une borne fixe (\figref{fig:automate_chauffage}). Dans le cas
du $Solaire_{direct}$, la température en sortie des capteurs ($T1$) doit être supérieure
ou égale à \SI{40}{\degree} et à la température de l’eau en sortie de
...
\fnref{http://www.solar-rating.org/}{\textit{ICC-SRCC}}) et seuls des capteurs récents sont considérés.
...
est réduite. Le calcul de la répartition des capteurs \abr{PV} est donc simplifié.
...
dans le \tabref{tab:carac_vitrages}. Les caractéristiques des vitrages sont ici présentées
...
pour des bâtiment du niveau \abr{RT\,$2012$} et des bâtiments à énergie positive notamment à
...
avec une surface de capteurs \abr{PV} importantes ne sont pas atteignables.
En considérant la minimisation de la $Conso_{tot}$ en place de la $Conso_{app}$,
un des objectifs tiendrait compte du nombre de capteurs \abr{PV}.
Cependant toujours d’après \defref{def:dominance_de_pareto}, le problème reste entier.
...
Finalement le dernier objectif, minimiser le $Nombre_{PV}$, est introduit afin de proposer
...
et de capteurs \abr{PV} est cependant obligatoirement un entier. Afin de contourner
...
la part passive (pertes des ballons, $Prod_{sol}^{CH}$ passive) (\figref{fig:prod_sol_chauffage_mu}).
...
En effet, augmenter le volume du ballon \abr{ECS} améliore la part solaire
...
La complexité liée à l’évaluation couplée d’un \abr{SSC} et d’un bâtiment a été exposée.
...
Au cours du chapitre II, il a été montré que le \abr{SSC}, en fonction des
caractéristiques des ballons (position de l’échangeur ou volume des ballons), réparti
l’énergie solaire entre chauffage et production d’\abr{ECS} de manière différente sans
...
\abr{SSC} couvre les besoins en chauffage. En effet pour une même valeur de $F_{sol}^{CH}$,
...
Finalement l’existence d’un compromis entre qualité de l’enveloppe et performance
du \abr{SSC} sur le chauffage, est aussi identifiée.
...
\abr{ECS} est un facteur important dans les deux climats alors
...
chauffage et à la production d’\abr{ECS} (\figref{fig:conso_app_mu_star}).
...
besoins pour la production d’\abr{ECS}, la consommation totale ($Conso_{app}$) est
...
permettant au \abr{SSC} de couvrir une part plus conséquente des besoins et donc de diminuer
...
Concernant la consommation totale, le nombre de capteurs \abr{PV} est sans surprise, le
...
est le facteur le plus influent. Ainsi, sur Strasbourg où les besoins en \abr{ECS} et
...
valorisée ($Prod_{sol}^{valorisée} = Prod_{sol} - Pertes_{réseau}$) (\figref{fig:prod_sol_valorisee_mu_star}).
...
aussi un impact important et le réduire augmente les pertes en ligne (annexe, \figref{fig:prod_sol_valorisee_mu}).
...
dans \tabref{tab:facteur_retenues}. Pour l’indicateur $Conso_{app}$, il est retenu
...
\begin{itemize}
  ...
  \item Les variations du \abr{SSC} sont plus influentes lorsque la $Conso_{app}$
...
% ..............................................................................
% ..............................................................................
\section{Construction des modèles de substitution} % (fold)
...
compter \SIrange{1}{3}{h} pour simuler le modèle du \abr{SSC} et \SIrange{10}{30}{min} pour
...
du système de référence et du \abr{SSC}, les deux modèles restent indépendants. Il est
...
le \abr{SSC} pour les deux climats ($2 \times 2$ échantillons). Les solutions sont
...
\fnref{http://openturns.org/}{\textit{OpenTurns}} et les travaux de \textcite{Merheb2013}
...
d’obtenir une bonne approximation (\figref{fig:mae_rmse_qualite_ref}). L’ordre du méta-modèle
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Ressources/Images/MetaModele/RMSE_ref.pdf}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Ressources/Images/MetaModele/MAE_ref.pdf}
        \caption{}
...
En effet, à partir d’un échantillon de taille $100$, le méta-modèle est d’ordre trois et précis à
...
Le modèle du \abr{SSC} étant plus complexe, un nombre plus important de simulations est
...
\begin{figure}
    ...
    \caption[Évolution de la $RMSE$ sur le \abr{SSC} en fonction de l’échantillon]
            {Évolution de la $RMSE$ pour les trois méta-modèles caractérisant le \abr{SSC}
...
Finalement l’échantillon met déjà en avant l’intérêt d’un \abr{SSC}, en particulier sur
...
% ..............................................................................
% ..............................................................................
\section{Optimisation multi-objectifs} % (fold)
...
% ------------------------------------------------------------------------------
\subsection{Qualité de l’approximation sur les fronts de Pareto} % (fold)
...
la couverture du chauffage et de l’\abr{ECS} de manière distincte permet de renforcer le
...
l’appoint est en effet déjà très faible et la production des capteurs \abr{PV} est
...
capteurs \abr{PV} sur le pan sud et donc d’améliorer la production.

Afin d’apprécier l’étendue du front de Pareto, les bornes de l’espace des objectifs,
...
Sur Bordeaux, le \abr{SSC} permet avec cinq capteurs de réduire de
...
De plus, le potentiel du \abr{SSC} semble plus élevé sur Strasbourg car les besoins
en chauffage comme pour la production d’\abr{ECS} sont plus importants.
Les parties suivantes analysent plus en détail le comportement du \abr{SSC} avec dans un
...
est elle plus importante. De ce fait, le \abr{SSC} permet de substituer un nombre plus important de
...
\abr{PV} doivent produire plus sur Strasbourg afin de compenser l’énergie non économisée
...
atteindre un bilan positif, que la production des capteurs \abr{PV} ($Prod_{PV}$) couvre
...
Les résultats montrent aussi que le nombre de capteurs \abr{PV} diminue lorsque la
performance du \abr{SSC} augmente. Afin de respecter la contrainte de bilan positif,
...
permet de réduire de huit (\SI{13.1}{m^{2}}) le nombre de capteurs \abr{PV} nécessaires
(\figref{fig:fsav_pv_stras}). Sur Bordeaux ajouter trois (\SI{7.0}{m^{2}}) capteurs
thermique permet de réduire de cinq (\SI{8.2}{m^{2}}) le nombre de capteurs \abr{PV}
(\figref{fig:fsav_pv_bor}). De plus, la production des capteurs solaires thermiques est en
adéquation avec la demande alors que le bilan \textit{production / consommation} est
réalisé à l’échelle annuelle pour le \abr{PV}. Il est aussi mis en évidence que les
solutions encadrées sont celles qui minimisent le nombre de capteurs \abr{PV} sur la
...
temps la surface minimale de capteurs \abr{PV}.
...
qui a pourtant le rendement optique ($\eta_{0}$) le moins élevé, permet au \abr{SSC}
...
capteurs par contre le volume du ballon d’\abr{ECS} est moins impactant. Aussi,
...
capteurs \abr{PV} varie aussi en fonction du nombre de capteurs solaires installées.
En effet pour un même nombre de capteurs \abr{PV}, la production
...
minimum quatre capteurs pour que le \abr{SSC} récupère assez d’énergie solaire pour améliorer le
...
inférieur à \SI{100}{\litre} avec trois ou quatre capteurs (annexe, \figref{fig:tanks_variations_bordeaux}).
...
Afin d’évaluer l’impact de la taille des ballons, une nouvelle optimisation
...
Par exemple pour sept capteurs, la \abr{SSC} permet au maximum, de couvrir
...
Ces résultats sont encourageants et invitent à repenser la conception d’un \abr{SSC} pour
...
% ------------------------------------------------------------------------------
\subsection{Influence des autres paramètres} % (fold)
...
production d’\abr{ECS} mais limite grandement la qualité de couverture du solaire pour le
...
% subsection influence_des_autres_parametres (end)
...
la $Conso_{app}$. L’analyse de la \figref{fig:conso_ref_vs_app} montre qu’il existe une
...
toujours une bonne idée, pourtant l’observation de la \figref{fig:conso_ref_vs_diffconso} permet
...
En conclusion, le choix du niveau d’isolation doit bien être déterminé parallèlement au choix
et au dimensionnement des
équipements du \abr{SSC}. Un autre indicateur doit être considéré afin
...
Afin d’évaluer les différences entre les différentes solutions,
...
\begin{blockdescription}{Solution 1 (\abr{S1})~:}
  ...
  \item[Solution 4 (\abr{S4})~:] Cette solution correspond à la solution maximisant le $F_{sav,\,ext}$ lorsque
                                 ...
                                 Le \abr{SSC} permet de couvrir près de \SI{43}{\percent} avec seulement deux capteurs
                                 solaires thermiques mais l’économie de cinq capteurs thermiques nécessite l’ajout
                                 de huit capteurs \abr{PV} supplémentaires. Enfin
...
Des résultats encourageants pour le développement de \abr{SSC} pour les maisons à énergie
...
du chauffage et de la production d’\abr{ECS}, car maximiser l’un des deux ne permet jamais
...
performance du \abr{SSC} est identifié, le choix restant propre aux contraintes du décideur~: sur-isoler
