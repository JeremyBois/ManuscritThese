...
\begin{itemize}
  ...
  \item L’influence du volume des ballons, tampon et d’\abr{ECS}.
  \item Les corrélations entre niveau d’isolation et performance du \abr{SSC}.
\end{itemize}
...
de capteurs \abr{PV} sur le pan sud et donc, améliorer la production.

Afin d’apprécier l’étendu du front de Pareto, il est aussi renseigné les bornes de
l’espace des objectifs (\tabref{tab:bornes_front_pareto}) correspondant au point nadir
...
et montre que le \abr{SSC} a aussi du potentiel sur un climat plus rude~:
\begin{itemize}
...
où le \abr{SSC} est moins performant. Ainsi, le nombre minimal de capteurs \abr{PV} est
...
thermiques de $5$ permet de réduire de $8$ le nombre de capteurs \abr{PV} nécessaires
(\figref{fig:fsav_pv_stras}). Sur Bordeaux ajouter $3$ capteurs thermique permet
...
(\SI{-304}{kWh_{ef}}) et en même temps la surface minimale de capteurs \abr{PV}.
...
entre économie sur le chauffage et économie sur l’\abr{ECS}. La production d’\abr{ECS} étant
...
de la production des capteurs \abr{PV} en fonction du nombre de capteurs solaires
installées. En effet pour un même nombre de capteurs \abr{PV}, la production varie
faiblement (\SI{\sim 100}{kWh}) car la toiture est partagée.
La performance du \abr{SSC} augmente à mesure que l’on augmente le nombre de capteurs
...
$3$ capteurs il soit plus pertinent de couvrir majoritairement la production d’\abr{ECS}
et que à partir de $4$ le \abr{SSC} récupère assez d’énergie solaire pour améliorer
...
La \figref{fig:evolution_front_pareto_nb_th} permet de mieux apprécier l’évolution de la
...
Il existe une large variabilité dans les valeurs des critères de décisions de l’algorithme
de contrôle.
...
apparaît que un $\Delta min_{capteur}$  faible est plus profitable. Au niveau global,
du fait des compromis, de nombreuses combinaisons sont possibles.

Pour obtenir la meilleure performance globale ($F_{sav,\, ext}$), il est
nécessaire de considérer un $\Delta T_{sol}$ faible (\SIrange{0}{10}{\celsius}).
...
de l’échangeur autour de \num{0.9}. Il n’est identifié de relations entre la position de l’échangeur
et les variables de décisions sur l’algorithme ($\Delta min_{capteur}$, $\Delta min_{tampon}$,
$\Delta T_{sol}$).


\ftodo{Ajouter cluster dans XDAT avec des noms proches du manuscrit.}
...
l’enveloppe est évaluée de manière qualitative, puis il est exploré les relations pouvant
exister entre $Conso_{ref}^{CH}$ (qui traduit la performance de l’enveloppe) et la
performance du \abr{SSC}.

Comme le montre \figref{fig:pareto_enveloppe_strasbourg}, le meilleur taux d’économie
est obtenue lorsque l’isolation du bâtiment est importante.
Pour le chauffage, il apparaît possible
d’obtenir un taux d’économie important sans pour autant fortement isoler le bâtiment
cependant, les meilleures solutions assument une isolation importante.
...
Dans le premier cas, une enveloppe peu performante permet d’obtenir le meilleur $F_{sav,\,
ext}$~: il n’y a pas assez d’énergie solaire pour permettre d’optimiser à la fois le
...
la production d’\abr{ECS} tout en ayant encore assez d’énergie pour fournir suffisamment
d’énergie sur le chauffage. Le solaire ne couvrant pas l’ensemble des besoins en
\abr{ECS}, réduire les besoins de chauffage permet de réduire la part solaire attribuée au
chauffage et donc il est possible de couvrir plus efficacement les besoins en \abr{ECS}.
...
Afin de mieux caractériser les corrélations existantes, $3$ indicateurs sont introduits, $p$,
...
% \ftodo{Type capteur et volume ballon  / performance XDAT}
% Les analyses précédentes ont montrées qu’il est possible d’obtenir une performance
% similaire pour différents types de capteurs et pour différentes tailles de ballons.
% Afin d’illustrer les propos tenus, une représentation paramétrique est proposée (\mtodo{ref fig}).
% La présente représentation permet d’évaluer simultanément l’évolution de la qualité de
% l’enveloppe en fonction du volume des ballons en construisant plusieurs clusters.
...
Le processus d’aide à la décision interactif est la dernière étape de la
...
forcement les mêmes que durant l’optimisation. En effet l’avantage d’une
...
coordonnées parallèles qui permet comme décrit dans le chapitre précédent de bouger des
curseurs pour chaque critère afin de réduire le nombre de solutions optimales. L’outil étant graphique, il
...
individuelles, il est possible de choisir la maison directement avec les clients.
...
de manière active aux choix constructifs et des systèmes sans avoir besoin
d’une expertise importante. Le client devient acteur de son choix et évalue mieux
les contraintes et atouts de chaque proposition; le constructeur est lui mieux à même de
présenter et défendre ces offres. De la même manière, ce processus est
adapté dans le cas où le respect d’une réglementation spécifique est nécessaire
et encore d’autres applications\dots

La large variété de solutions peut être observée à travers \mtodo{ref fig}.
Prenons le cas où le client souhaite limiter au maximum la place prise par les équipements.
En jouant sur les curseurs, il est possible de conserver uniquement les solutions
qui correspondent et de voir directement les effets sur les autres critères de décisions
ou les objectifs (\mtodo{ref fig}).
Étant un peu juste financièrement, il souhaite
aussi limiter le nombre de capteurs solaires thermiques sans augmenter le nombre
de capteurs \abr{PV}(\mtodo{ref fig}). Finalement il trouve une solution qui lui
correspond.
À partir de là, il est possible de relancer le processus d’optimisation
en tenant compte de la préférence du décideur afin de proposer plus de choix sur les
autres critères.

\begin{figure}
    \centering
    \ftodo{Figure avec évolution du front en fonction des préférences (3 stade)}
    % \includegraphics[width=\textwidth]{Ressources/Images/EtudeDeCas/Strasbourg_front.pdf}
    % \includegraphics[width=\textwidth]{Ressources/Images/EtudeDeCas/Strasbourg_front.pdf}
    % \includegraphics[width=\textwidth]{Ressources/Images/EtudeDeCas/Strasbourg_front.pdf}
    \caption[None]
             {Évolution de l’aide à la décision en fonction des choix}
    \label{fig:xdat_exemple}
\end{figure}


Six solutions ont été sélectionnée qui permettent de récapituler les observations faites
...
\begin{blockdescription}{Solution 1 (\abr{S1})~:}
  \item[Solution 1 (\abr{S1})~:] Cette solution maximise le $F_{sav,\,ext}$. Elle représente
                                 le meilleur compromis entre une bonne couverture du chauffage
                                 et de la production d’\abr{ECS} et permet donc minimiser
  ...
  \item[Solution 2 (\abr{S2})~:] Cette solution maximise le $F_{sav}^{ECS}$ et est très
                                 ...
                                 la solution minimiser $F_{sav}^{CH}$ notamment avec l’utilisation
  ...
  \item[Solution 3 (\abr{S3})~:] Cette solution maximise le $F_{sav}^{CH}$. $F_{sav}^{ECS}$ reste
                                 ...
                                 et les besoins en \abr{ECS} reste similaires durant l’année entière.
                                 À l’inverse de la solution $2$, c’est cette fois le ballon sanitaire qui
                                 est minimisé. Par contre il reste à \SI{200}{\litre} alors que la valeur
                                 minimale est de \SI{100}{\litre}. C’est aussi la seule solution où
                                 l’échangeur est plus haut dans le ballon mais pas à la hauteur maximale.
                                 En augmentant sa position, la production solaire sur l’\abr{ECS} est défavorisée
                                 et une part plus importante est utilisée pour le chauffage.
  \item[Solution 4 (\abr{S4})~:] Cette solution correspond à la solution maximisant le $F_{sav,\,ext}$ lorsque
                                 ...
                                 solaires thermiques mais l’économie de $5$ capteurs thermiques nécessite l’introduction
                                 ...
                                 contrairement à l’appoint économisé par le \abr{SSC}.
  \item[Solution 5 (\abr{S5})~:] Cette solution maximise le $F_{sav,\,ext}$ mais
                                 est contraint par l’utilisation d’un volume faible
                                 pour les deux ballons. Mis en exergue dans \ref{sub:influence_du_volume_des_ballons},
                                 le volume des ballons peuvent être réduit tout en permettant de
                                 conserver une bonne performance globale.
  \item[Solution 6 (\abr{S6})~:] Cette solution maximise le $F_{sav,\,ext}$ mais
                                 est contraint par l’utilisation d’un capteur
                                 \textit{IDMK}. Comme explicité en \ref{sub:influence_des_capteurs_solaires_thermiques}
                                 le type de capteur est peu influent. De plus comme explicité
                                 dans \ref{sub:influence_du_volume_des_ballons}, les paramètres
                                 de l’algorithme sont très variants et il existe des solutions
                                 similaires sur le $F_{sav,\,ext}$ mais pour des configurations
                                 différentes.
  \item[Solution 7 (\abr{S7})~:] Représente la solution qui maximise le $F_{sav,\,ext}$
                                 lorsque uniquement $5$ capteurs sont installés. L’isolation
                                 est amélioré par rapport à la solution $1$ mais la consommation
                                 de l’appoint augmentée de \SI{300}{kWh}. L’écart nécessite
                                 donc l’installation de $3$ capteurs \abr{PV} supplémentaires.
  \item[Solution 8 (\abr{S8})~:] Représente la solution qui maximise le $F_{sav,\,ext}$
                                 lorsque le niveau d’isolation est inférieur à \SI{50}{\percent}
                                 que ce soit sur le plancher, le plafond, ou les murs.
                                 Lorsque l’isolation est peu importante, c’est le ,
...
\begin{tabular}{l c c c c c c c c}
  ...
  \midrule
  Nombre capteurs \abr{TH}      & \num{7} & \num{7} & \num{7} & \num{2} & \num{7} & \num{7} & \num{5} & \num{7}  \\
  Type capteurs \abr{TH}        & \multicolumn{5}{c}{\textit{SkyPro}} & \textit{IDMK} & \multicolumn{2}{c}{\textit{SkyPro}}  \\
  $Ech_{sol}^{pos}$             & \num{0.8} & \num{0.96}  & \num{1.23}  & \num{0.95}  & \num{0.94}  & \num{0.87}  & \num{1} & \num{1.21}  \\
  Volume ballon tampon          & \num{400} & \num{100} & \num{400} & \num{250} & \num{200} & \num{400} & \num{400} & \num{400}  \\
  Volume ballon $ECS$           & \num{400} & \num{400} & \num{200} & \num{350} & \num{200} & \num{400} & \num{400} & \num{400}  \\
  $\Delta T_{sol}$              & \num{5} & \num{10}  & \num{8} & \num{10}  & \num{10}  & \num{18}  & \num{0} & \num{7}  \\
  ...
  Surface vitrée sud    & \num{5.8} & \num{6.6} & \num{6,2} & \num{5.4} & \num{6.2} & \num{5.4} & \num{5.8} & \num{5.7} \\
  Surface vitrée est    & \num{4.3} & \num{4.7} & \num{4,3} & \num{4.7} & \num{4.4} & \num{4.6} & \num{4.3} & \num{5.7} \\
  Type de vitrage       & \multicolumn{3}{c}{\textit{PlanithermXN}} & \textit{PlanithermOne}  &\multicolumn{3}{c}{\textit{PlanithermXN}} & \textit{OptiWhiteKGlass}  \\
  \\
  \addlinespace[\defaultaddspace]
...
\end{tabular}
\end{table}
% subsection aide_a_la_décision_a_posteriori (end)

Comme explicité en \ref{sub:correlation_entre_performance_de_l_enveloppe_et_ssc} comme il n’est
évalué uniquement les caractéristiques énergétiques, un biais est donc introduit favorisant
uniquement des solutions avec une forte isolation. Pour cette raison il est nécessaire
d’introduire un autre objectif tel que le confort intérieur, ou le temps de retour sur l’investissement,
afin de voir apparaître des caractéristiques de l’enveloppe plus diverses. Sur les autres paramètres,
une large diversité est obtenue.
...
\abr{SSC} appliqués aux maisons à énergie positive. Une approche exploratoire des
différents objectifs liés à la performance énergétique s’est en effet montrée être
pertinente pour mieux comprendre les interactions existantes. Cependant l’approche retenue
introduit un élitisme important sur certains critères de décisions en partie car tous les
objectifs ne sont pas antinomiques. Il a donc été proposé de mettre en place un processus itératif d’aide à la
décision. Dans un premier temps l’optimisation est réalisée en considérant l’ensemble des
critères de décisions. Afin de ne pas écarter trop tôt des solutions et explorer l’ensemble du
domaine de décision aucun caractère préférentiel est introduit. Ensuite en fonction des objectifs secondaires et des résultats de la
...
aussi aux contraintes propres du ou des décideurs. Elle reste aussi adaptée pour des
...
% subsubsection exploration_des_solutions_pour_le_climat_de_strasbourg (end)
...
invitent à repenser les choix de conception lors du développement de \abr{SSC} couplés à
des \abr{MEPOS}.
