...
\paragraph{} % (fold)
À travers ces travaux, il a été mis en avant que la construction d’une \abr{MEPOS} est un
...
De par sa simplicité, le solaire thermique et par extension le \abr{SSC} est un
...
sous \textit{EnergyPlus}. Concernant le \abr{SSC}, le système développé utilise l’air
...
résultats aient permis d’identifier le potentiel du dimensionnement couplé d’une \abr{MEPOS} et d’un
...
le comportement des abeilles mellifères (\abr{ABC}) est retenue pour l’optimisation principalement car
...
développement de \abr{MEPOS} ainsi que la diversité des solutions pouvant être obtenues.
...
occupants est un facteur important lors de l’évaluation de la performance des \abr{MEPOS}.
...
nécessaire de vérifier que le modèle traduit correctement le comportement du \abr{SSC}
...
faisant circuler l’eau du \abr{SSC} dans les capteurs solaires en période nocturne.

Ces travaux ont aussi contribué au développement d’une méthodologie d’aide à la décision
...
la variation de la performance du \abr{SSC} pour une période étendue de simulation. Cette
...
possible de quantifier le potentiel énergétique et économique du \abr{SSC} pour différents
...
l’évaluation du gain de performance d’un \abr{SSC} est directe. Il est cependant important
...
îlot de maisons individuelles. Avec la \abr{RT\,$2012$}, les frais de chauffage sont
maintenant obligatoirement individualisés mais il est toujours possible d’opter pour un
système collectif pour le chauffage et la production d’\abr{ECS} en ajoutant des compteurs
...
% Respecter trame méthodologie
% limites
% enjeux cour terme
% enjeux long terme

% Occupants charges internes  ---> Vorger

% Interface pour l’utilisation

% Illustrée avec un \abr{SSC} utilisant le vecteur air et une batterie électrique, la
% méthodologie reste applicable pour d’autres équipements ...
% familles de systèmes

% Généricité algo -- > développement modèles de coût -- > implémentation




% rénovation lourde, nécessite le développement de plein de modèles,
% Tout faire en même temps
% contraintes architecturales
% Cas de référence == départ
% Potentiel solaire
% Son adaptation nécessite cependant comme
% pour le collectif de tenir compte de nouvelles contraintes comme la séquentialité du
% processus de rénovation.


% un effort de modélisation est nécessaire pour prendre en compte l’échelle collectif
% Algorithme générique (discret / qualitatif / continu) permettant une application
% pour différentes échelles, le travail devant se focaliser sur la création de
% modèles de comportement adaptés pour l’échelle visée
% Choix des objectifs car la nature du front est fortement dépendant.
% Tenir compte du foissonnement de l’énergie
...
% 5min présentation
% Potentiel solaire
% Penser que les gens dans la salle sont pas experts --> vision globale
