...
\caption{Différences principales entre les différents modèles modélisés dans la tâche\,$26$.
         Le CHauffage est désigné \abr{CH} et l’Eau Chaude Sanitaire par \abr{ECS}.}
\label{tab:diff_ssc}
\begin{tabular}{l C{2cm} C{1.5cm} C{0.6cm} C{1.4cm} C{1.7cm} C{1.7cm} C{1.2cm} C{1.5cm} C{1.2cm} C{2.5cm} C{1cm} r}
    \toprule
            & Type de Chaudière    & Appoint électrique  & \abr{CH} mixte  & \abr{CH} et \abr{ECS} ensemble & Type $Prod_{CH}$ & Type $Prod_{ECS}$ & Débit  variable & Contrôleur unique \abr{CH} et \abr{ECS} & $Ech_{sol}$ externe & Nombre d’échangeurs & Nombre de ballons & Référence \\
    \midrule
