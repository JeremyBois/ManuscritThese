...
de l’Agence Internationale de l’Énergie (\abr{AIE}) \parencite{Athienitis2015}. Les travaux se focalisent
...
considérée à \SI{80}{ans}.\textcite{Sartori2012220} propose trois types de bilan (\figref{fig:bilan_zeb})~:
\begin{itemize}
...
\paragraph{Généralités~:} % (fold)
\label{par:generalites}
...
d’obtenir une fraction solaire ($F_{sol}$, \eqref{eq:f_sol}) plus importante d’un tiers comparé à un ballon dont
...
\begin{subequations}\label{eq:f_sav_sol}
    \begin{align}
        \label{eq:f_sav}
        F_{sav} &= 1 - \frac{Conso_{app}}{Conso_{ref}} \\[10pt]
        \label{eq:f_sol}
...
% paragraph generalites (end)


\paragraph{La tâche $\bf{26}$~:} % (fold)
\label{par:la_tache_26}
...
\end{figure}
% paragraph la_tache_26 (end)


\paragraph{Remarques fondamentales~:} % (fold)
\label{par:remarques_fondamentales}
...
\textcite{} montre lui aussi à l’aide d’un modèle analytique journalier que le volume
...
chaudière biomasse couplé à des capteurs solaires. Les résultats acquis hors période
...
solaire saisonnal à travers $4$ configurations couplés à une \abr{PAC} pour couvrir les
...
% paragraph remarques_fondamentales (end)

\paragraph{Optimisation des \abr{SSC}~:} % (fold)
\label{par:optimisation_des_ssc}
...
successives pour un ou plusieurs paramètres définies par l’ingénieur, l’optimisation
explore l’espace des solution de manière automatisé. \textcite{Fraisse2009232} présente
...
évalué successivement suivant sa performance d’un point de vue énergétique (exergie et
$F_{sav, ext}$, $F_{sav, si}$), économique (coût du cycle de vie, \abr{LCC}), et
écologique (émissions de $CO_{2}$ puis en combinant plusieurs objectifs. L’exergie permet
de quantifier l’utilité d’une énergie qui diminue au cours de ces transformations. La
valeur minimiser est donc la somme de l’énergie détruite par transformations successives,
l’anergie. La définition est donc fortement dépendante de la source primaire considérée
(référence). Le système est décrit dans \textit{TrnSys} et optimisé grâce à
...
\begin{itemize}
    ...
    \item Minimiser la consommation de l’appoint ($F_{sav, ext}$)
    \item Minimiser la combinaison coût et économie d’énergie
...
bâtiment, les remarques restent transposables en parti aux \abr{SSC}.
...
réalisé sur le coût sur le cycle de vie et il est montré que coupler la \abr{PAC} avec le
...
le \abr{LCC}, le cumul de l’énergie grise et de l’énergie consommée sur la durée de vie
...
Optimiser en fonction de \abr{LCC} le système optimal obtenue considère un unique capteur
...
Cependant la première approche permet d’obtenir un \abr{LCC} $\nicefrac{1}{3}$ inférieur à
la solution optimisée sur le \abr{LCE}. \textcite{Bornatico201231} utilisent eux le
logiciel \textit{PolySun} pour évaluer un \abr{SCC} à travers le taux de couverture, la
consommation totale, et le coût d’investissement aggloméré en un unique objectif. Le
...
\textcite{Asaee2014510} évalue la performance d’un \abr{SSC} (système \#14 de la tâche
...
$4$ climats étudiés avec un taux de couverture supérieur à \SI{70}{\percent}. Finalement
...
composant le \abr{SSC} sont retenue comme critère de décisions. L’optimisation mono-
objectif est réalisée en réalisant une somme pondérée des différents objectifs (\abr{LCE}
et \abr{LCC}). L’étude montre que l’obtention de solution résultant d’un compromis entre
les objectifs est un travail fastidieux et couteux en temps lorsque l’approche mono-
objectif est utilisée.
% paragraph optimisation_des_ssc (end)
...
est très variable et de nombreux facteurs influents ont été identifiés et leur sensibilité
analysé comme l’inclinaison des capteurs ou le volume du ballon de stockage. Il est
cependant aussi observé que la majorité des études ne considèrent que les différents
équipements principaux du \abr{SSC} (capteurs, volume des ballons)~;la qualité de
l’enveloppe et la logique de contrôle étant définies en amont et fixé même lors des études
d’optimisations. La littérature met aussi en évidence qu’il existe de nombreux objectifs
et que la performance varie fortement en fonction de celui-ci. Il est aussi mis en
évidence la forte disparité des résultats obtenue pour le temps de retour sur
investissement qui varie de \SIrange{5}{100}{ans} \parencite{Tsalikis2015743,Hin2014102}.
Sur l’évaluation de la performance énergétique de nombreux indicateurs existent
(\eqref{eq:indicateur_performance}, \eqref{eq:f_sav_sol}, exergie). Il est aussi noté que
suivant l’indicateur considéré, des précautions doivent être prise afin d’éviter une
...
réglementations sont misent en place au $20^{ème}$ siècle afin de sécurisé
...
employé à travers une approche par essai/erreur. De part sa rapidité de calcul, il offre
la possibilité d’explorer un nombre important de solutions et de vérifier des
suppositions plus rapidement. À partir d’un cas de référence généralement défini par
...
rapidement aux marchés dont la complexité et la combinatoire augmentent parallèlement
aux réglementations.
...
Parallèlement, il a été mis en évidence que solaire thermique est une technologie ancienne
...
outils permettant une approche détaillée du \abr{SSC} comme \textit{TrnSys}. Cependant
...
varient fortement. Partant de ces informations, il apparaît plus intéressant de proposer
...
objectif et multi-objectifs est discuté plus amplement dans le chapitre $3$ de ce
document.
...
\abr{SSC}, la logique de contrôle, et l’enveloppe du bâti. La méthodologie s’inspirent des
contraintes décrites à travers la norme \abr{PREN\,$15603$} afin d’aider au développement
des \abr{BEPOS} solaires adaptée aux contraintes du secteur du bâtiment. S’appuyant principalement sur
...
de substitutions. Dans un second temps l’optimisation multi-critère est présentée
et une analyse circonstanciée des méthodes d’optimisation est réalisé afin de justifier
...
en parallèle pour des conditions climatiques distinctes.
