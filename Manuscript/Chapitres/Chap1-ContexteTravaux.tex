...
Universel ou temps solaire (\abr{UT1}). Tenant compte de la position du site étudié
l’heure solaire locale (\abr{HSL}) peut être calculée de manière simplifié avec
...
le \abr{MIR}. La combinaison des deux permet alors de récupérer la majorité du spectre
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.55\textwidth}
        \includegraphics{Ressources/Images/Solaire/capteur_plan_vitré.jpg}
        \caption{}
...
paraboliques (\figref{fig:capteur_cylindro_parabolique}) utilise des réflecteur
...
\paragraph{Les travaux en cours~:} % (fold)
...
retombées.
% paragraph les_travaux_en_cours (end)
...
solaire. Le \abr{SSC} est ainsi aujourd’hui une technologie principalement utilisé dans la rénovation
...
locale par type d’énergie laissant présager d’un futur pour les \abr{SSC}.
% paragraph conclusion (end)
% subsubsection les_chiffres_cles (end)


\subsubsection{D’hier à aujourd’hui} % (fold)
...
Une des majeure contribution aux \abr{SSC} pour la maison individuelle a été réalisé au cours
...
Enfin un dernier indicateur est introduit le \abr{FSC} \eqref{eq:FSC} (Fractional Solar Consumption)
...
technologies et logiques de contrôle modélisés (\tabref{tab:diff_ssc}).
...
une chaudière au gaz, un échangeur à plaques pour l’\abr{ECS}, et le ballon de stockage.
...
directe. En effet dans les autres systèmes l’\abr{ECS} est puisé directement du ballon.
...
\caption{Différences principales entre les différents modèles modélisés dans la tâche\,$26$
         avec \abr{CH} pour CHauffage et \abr{ECS} pour Eau Chaude Sanitaire.}
\label{tab:diff_ssc}
\begin{tabular}{l *{6}{c} *{5}{G} G}
    \toprule
            & Type & Appoint    & \abr{CH}  & \abr{CH} et \abr{ECS}  & Type     & Type      & Débit    & Contrôleur unique       & $Ech_{sol}$  & Nombre      & Nombre & \\
            & Chaudière & électrique & mixte     & simultanés        & \abr{CH} & \abr{ECS} & variable & \abr{CH} et \abr{ECS}   & externe    & d’échangeur & de ballons & \\
    \midrule
    \bottomrule
\end{tabular}
\\[10pt]
...
\itodo{Bilan}
Les \abr{SSC} existent ainsi sous de nombreuses variations utilisant soit des chaudière,
soit des \abr{PAC}, soit simplement un appoint électrique. La performance de ces
...
différent \abr{SSC} pour différents climats ou surface de capteurs. Il est cependant noté que
peu de travaux on cherché à étudié le comportement de ces systèmes pour des \abr{BEPOS}
...
% subsubsection d_hier_a_aujourd_hui (end)
...
\iunsure{plus dans bilan et description de l’approche}
...
acquise dans l’optique de satisfaire aux besoins immédiats du client. Les problèmes de
changement climatique couplés à la prise de conscience d’une quantité limitée d’énergies
fossiles ont introduit un changement dans la construction à travers des réglementations
thermiques visant à de plus en plus d’exemplarité. Le secteur du bâtiment qui est
aujourd’hui considéré comme le secteur consommant le plus d’énergie est aujourd’hui vu
comme une part d’un ensemble considérant le confort des occupants, les limitations des
émissions de $CO_{2}$, la limitation de la consommation en énergie primaire.
...
employé à travers une approche par essai/erreur. De part sa rapidité de calcul, il offrant
ainsi la possibilité d’explorer un nombre important de solutions et de vérifier des
suppositions plus rapidement. À partir d’un cas de référence généralement définie par
l’expérience, un ensemble de variations sont introduites à l’aide d’itérations
successives. Ce processus est couteux en temps humain et ne permet pas de valoriser la
...
rapidement aux marchés dont la complexité et la combinatoire augmente.

Ainsi afin de pouvoir répondre au nouvelles contraintes du marché, le temps humain doit
être valorisé pour des tâches cognitives et le temps machines assujetti aux tâches
...
phase de conception grâce à l’automatisation, l’intelligence artificielle qui impose le
développement d’une méthodologie adaptée.
...
La construction d’un bâtiment est par définition multi-critère, elle fait intervenir un
...
~\\ \itodo{Discuter les travaux existants sur le solaire thermique}
...
~\\ \itodo{Discuter sur la norme pour introduire l’axe de recherche retenue}
...
la mise en place de labels certifiant une qualité supérieur. L’état est aussi un acteur
important à travers ces décisions politiques incitatrices en particulier sur la politique
...
actions combinés de tout ces intervenants qui permet aujourd’hui d’être à l’aube
d’un cadre réglementaire européen (\abr{prEN\,$15603$}) sur le \abr{BEPOS} dont chaque pays membres devra s’inspirer
pour la définition de leur nouvelle réglementation thermique.
Bien que la norme ne soit pour l’heure qu’un brouillon, les axes principaux sont
d’ores et déjà établis. La norme s’articule autour de quatre objectifs péremptoires~:
...
De plus comme l’indique la norme en construction à travers l’indicateur $RER$,
la consommation locale en énergie primaire devra être fortement couverte
par des énergies renouvelables dont l’élévation se fera par type d’énergie.
L’approche choisi à travers ces travaux est donc la minimisation de la consommation
d’énergie non renouvelable en couvrant au maximum la demande énergétique par le solaire
thermique. Il apparaît donc prioritaire de valoriser le solaire thermique par rapport
au solaire électrique, le photovoltaïque.
Ainsi dans ces travaux, la production d’énergie photovoltaïque
est considérée comme secondaire et doit permettre de couvrir les besoins électriques
de plus en plus importants dans les maisons individuelles. Il est ainsi évalué
à la fois les besoins de chauffage et d’\abr{ECS}, mais aussi les charges internes.
Même si la réglementation française ne considère pour le moment que les charges d’éclairage
l’approche retenue tient aussi compte des charges électro-domestiques qui sont majoritaires.

Les autres objectifs de la norme \abr{prEN\,$15603$} seront aussi pris en compte
...
consommation de l’électro-ménager, la consommation de l’éclairage.

~\\ \itodo{Conclure --> Bilan du bilan}
Au final, l’approche cherche à explorer et identifier des combinaisons pertinentes
entre système et enveloppe du bâti qui répondent aux contraintes explicitées ci-dessus
afin d’aider au développement de solution de \abr{BEPOS} solaires.



~\\ \itodo{Décrire l’architecture de la thèse}
