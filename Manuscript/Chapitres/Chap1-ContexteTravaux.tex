...
émissions de gaz à effet de serre (\abr{GES}).
...
de l’Union Européenne (\abr{EU}) définissent des objectifs globaux pour $2020$. Ces objectifs
sont au cœur de la directive 20/20/20 (\mtodo{Ajouter ref}) et engage l’Europe à réduire
de \SI{20}{\percent} des \abr{GES} (par rapport à $1990$), à améliorer de \SI{20}{\percent} l’efficacité énergétique et
...
de la tâche\,$26$ \parencite{Task262003}. La tâche s’articule en trois sous-tâches et
l’ensemble des modèles ont été modélisé à l’aide du logiciel \textit{TRNSYS}.
Dans la sous-tâche A, une méthodologie est définie pour le développement des modèles
numérique et le calcul des indicateurs permettant d’apprécier la performance au
regard des économies engendrés par rapport à un système de référence avec une
chaudière, soit au gaz, soit au fioul. $3$ indicateurs sont ainsi retenus, le
$f_{sav, therm}$, le $f_{sav, ext}$, et le $f_{si}$. Le premier est le taux d’économie
et caractérise la réduction de la consommation de l’appoint au regard du système
de référence. Le second ajoute au calcul la prise en compte des auxiliaires électriques
supplémentaires dues à l’ajout d’un système solaire. Le dernier



Dans la sous-tâche B, le nouveau modèle de capteur solaire implémenté dans TRNSYS,
(type 132) est discuté. Le rendement du capteur est définie à partir des équations
quasi-dynamiques explicitées dans l’ISO\,$9806$-$3$ qui définit la procédure
de test nécessaire pour les capteurs solaires non vitrés \mtodo{Ajouter ref}.
Cette procédure a été depuis implémenté de manière similaire au sein de la dernière
réglementation européenne en vigueur, la EN\,$12975$-$2$ \parencite{EN129752006}. Celle
ci propose est possible soit de calculé la performance du capteur à partir
de l’équation dite statique soit de l’équation dite quasi-dynamique. La première
est celle déjà définie à travers la EN\,$12975$ \parencite{EN129752002}


sous-tâche C, une analyse de sensibilité a été réalisé sur $9$ systèmes existants
...
évalué par rapport au même cas de référence et une analyse paramétrique est effectuée. La
sensibilité du système pour chaque paramètres est ensuite déduite de ces résultats.
Les systèmes considérés ont tours une spécificité propre et la tâche donne ainsi
...
de \textcite{Task262003,Shariah2002587}.

Au niveau de la surface, il est identifié que l’impact est moins important à mesure
que la surface augmente \parencite{Task262003}. En effet, la température du stockage augmentant les pompes
...
Toujours à travers les résultats de la tâche\,$26$ \parencite{Task262003}, l’ensemble
des systèmes considèrent un ballon unique à l’exception du système \emph{\#3a}.
...
Les résultats montrent que la performance du système augmente avec la réduction des
besoins en chauffage.



\itodo{Les travaux / technologies existants voir article}
...
\itodo{Task 40}
La tâche\,$40$ \mtodo{Ajouter ref} s’inscrit elle dans une démarche plus globale,
et considère le $BEPOS$ dans son ensemble en coopeération avec l’annexe $52$. Les
approches solaires passives comme actives y sont discutées de manière générale.
...
\begin{description}[align=left]
    \item [Programmation]
          ...
          sont réalisés.
    \item [Conception]
          Elle fait intervenir la \abr{MOA} et la maîtrise d’œuvre (\abr{MOE}). Des études technique \\
          Elle fait intervenir la \abr{MOA} et la maîtrise d’œuvre (\abr{MOE}). Des études technique
