...
% ------------------------------------------------------------------------------
\subsection{L’origine du réchauffement climatique} % (fold)
\label{sub:l_origine_du_rechauffement_climatique}
L’énergie (\defref{def:energie}) subit une évolution importante lors de la révolution
industrielle au début des années $1990$ avec le développement des machines consommant une
nouvelle forme d’énergie. Auparavant les différentes tâches étaient principalement
réalisées soit en utilisant les éléments (vent, eau, feu) soit en utilisant la force
animale (bœufs, chevaux\dots). La tendance s’accélère après la seconde guerre mondiale,
durant la période des $30$ glorieuses, où la France et d’autres pays développés profitent
d’une croissance forte tant au niveau économique, que démographique (baby-boom). Cette
expansion est portée par un accès aisé aux énergies et particulièrement aux énergies
...
énergétique explose.
Afin de pouvoir comparer les différentes énergies entres elles, une unité commune est
retenue~: le tonne équivalent pétrole (\abr{tep}) ou tonnes of oil equivalent (\abr{toe}) dans sa
version anglophone qui équivaut au pouvoir calorifique d’une tonne de pétrole, soit
\SI{41.868}{\giga\joule}.
...
reste aujourd’hui encore fortement majoritaire même si la production de pétrole
diminue.
...
% \begin{figure}
%     \centering
%     \includegraphics{Ressources/Images/Environnement/energy_region.png}
%     \caption{Évolution de la consommation d’énergie primaire mondiale par région
%              entre $1971$ et $2014$ d’après \textcite{IEA2016}.}
%     \label{fig:energy_region_evolution}
% \end{figure}


Une fraction de cette énergie est utilisée pour le chauffage, la voiture, ou encore pour
nos équipements électriques de plus en plus nombreux. L’autre fraction moins visible est
...
importante l’environnement. Les effets du réchauffement climatiques sont aujourd’hui facilement observables
et sont tels qu’ils sont capables de bouleverser le fonctionnement de la planète.
Bien que non imputable en totalité au réchauffement climatique, les évènements extrêmes
tels que l’augmentation des occurrences de canicules ou de fortes précipitations est fortement
lié à l’augmentation de la température mondiale moyenne. De même l’augmentation de la température moyenne
et du niveau des océans sont fortement corrélés (\figref{fig:evolution_climat}).
Le développement humain (transport, bâtiment, population, robotisation\dots)
et aussi responsable de nombreux autres facteurs environnementaux aggravants (acidification des sols, augmentation de la
température moyenne, pollution de l’air, réduction de la biodiversité\dots) \parencite{Biermann2016341}.
...
\begin{figure}
    ...
    \caption{Mise en évidence des effets anthropiques globaux et à l’échelle de la région
              ...
              des différentes simulations respectivement avec et sans influences humaines. Les zones
              ombrée en bleu et rose représentent les intervalles de confiance pour les différentes
              simulations. Finalement les lignes noires décrivent les observations \parencite{AchutaRao2013}.
             }
    \label{fig:evolution_climat}
\end{figure}

\ifix{Trouver sources plus récentes pour effet de serre}
La sur-utilisation des énergies fossiles couplée à une forte évolution démographique se
traduit par de trop fortes émissions en Gaz dits \enquote{à Effet de Serre (\defref{def:effet_serre})} (\abr{GES}).
La température moyenne sur l’ensemble de la surface du globe est ainsi le résultat du bilan radiatif entre le
rayonnement solaire incident et le rayonnement infrarouge terrestre émis.
L’eau contribuant à près de \SI{55}{\percent} à l’effet de serre apparaît comme
le composant principal. L’Homme n’ayant pas d’impact direct sur sa
quantité dans l’atmosphère, il n’est pas considéré lors de l’étude des émissions de
\abr{GES} anthropiques. Le quantité de $CO_{2}$ dans l’atmosphère, second composant
responsable du réchauffement climatique avec \SI{39}{\percent} est alors retenue.
Il représente près de \SI{74}{\percent} des effets anthropiques (\figref{fig:prop_gaz_serre}).


\begin{Def}[Énergie]\label{def:énergie}
...
pays, les facteurs de conversion sont eux aussi dépendant de chaque pays. Pour la France,
les coefficients souvent admis sont définies dans \tabref{tab:coef_finale_primaire}.
\end{Def}

\begin{table}
\centering
\ifix{Mettre ailleurs}
...
l’espace~: c’est le mécanisme d’effet de serre. Dans des conditions normales ce mécanisme
permet de maintenir une température correcte pour le développement de la biodiversité
alors que en son absence la température moyenne de la terre serait de \SI{-18}{\celsius}.
\end{Def}
...
\begin{figure}
    ...
    \caption{Répartition des émissions anthropiques responsable de l’effet de serre.}
    \label{fig:prop_gaz_serre}
\end{figure}

\itodo{Introduire GIEC}
...
Unies (\abr{ONU}) afin de fournir une base
...
augmentation anormalement rapide comme le confirme les données récentes (\figref{page 895}).
...
Il est ainsi clair que des mesures doivent être
...
% subsection l_origine_du_rechauffement_climatique (end)
...
\iunsure{Parler du projet DeserTec}
...
~
\itodo{Réglementation jusqu’à \abr{RT\,$2005$} pour introduire BBC}
L’abondance de ressources énergétique se traduit dans le bâtiment par des
...
Cette réglementation évolue ensuite en $1982$, $1989$, $2000$, et $2006$ avec la \abr{RT\,2005} porté par
...
du confort d’été.
Aujourd’hui la \abr{RT\,$2012$} en vigueur impose
des contraintes fortes sur la qualité énergétique des bâtiment au niveau de l’enveloppe
avec le \abr{$B_{bio}$} et sur les consommations avec le \abr{$C_{ep}$\,max}. Cette forte accélération
...
(\abr{2002/91/EC} \mtodo{Ajouter ref}) qui fut à l’origine
d’avancées significatives pour tous les états membres avec notamment la mise en place
des diagnostics de performance énergétique (\abr{DPE}), des valeurs seuils et une méthodologie commune
pour évaluer la performance énergétique des bâtiments, le contrôle régulier des chaudières (\SI{+10}{kW})
et un effort de normalisation commune par le Comité Européen de Normalisation (\abr{CEN}).
Cette directive a été mis à jour en $2010$ (\abr{2010/31/EU} \textcite{EPBD2010}) et introduit le concept
de \enquote{Nearly Zero Energy Buildings} (\textit{nearly}\,\abr{ZEB}) incitant chaque état membre à mettre
...
la \abr{RT\,$2005$} puis la \abr{RT\,$2012$}.
...
Basse Consommation (\href{https://www.effinergie.org/web/index.php/les-labels-effinergie/bbc-effinergie}{\abr{BBC-$2005$}})
...
\abr{RT\,$2012$}. En effet le passage à la \abr{RT\,$2012$} impose aux bâtiments un \abr{$C_{ep}$\,max} réduit
de moitié en plus de devoir tenir compte de l’éclairage et des auxiliaires alors que
dans la \abr{RT\,$2005$}, le \abr{$C_{ep}$\,max} est définie uniquement pour le chauffage, la production
d’\abr{ECS}, et le refroidissement.
Les travaux se concentre aujourd’hui sur la préparation de la \abr{RT\,2020}, qui devra
...
la directive européenne \abr{2010/31/EU} \parencite{EPBD2010} décrit un cadre commun pour les pays
de l’Union Européenne. Elle transcrit la volonté des États membres à construire pour $2020$
uniquement des bâtiments consommant \enquote{quasiment rien} (\textit{nearly}\,\abr{ZEB}) comme explicité ci-avant.
...
Énergie POSitives (\abr{BEPOS}) dans le cadre de sa propre réglementation. Communément,
...
l’a été le label \abr{BBC\,$2005$} en France pour la \abr{RT\,$2005$}, de nombreux projets pilotes
...
de la future réglementation, la \abr{RT\,2020}.
...
et \href{https://www.effinergie.org/web/index.php/les-labels-effinergie/bepos-effinergie}{\abr{BEPOS}-\textit{Éffinergie}}
permettant de valoriser une performance énergétique du bâtiment
supérieure au cadre réglementaire. Dans les deux cas la valeur maximale du \abr{$B_{bio}$},
du \abr{$C_{ep}$\,max}, de l’étanchéité à l’air ou de l’efficacité des équipements sont renforcées.
...
Le label \abr{BEPOS}-\textit{Éffinergie} va plus loin que le label \textit{Éffinergie\,+} en rendant
...
un bilan en énergie primaire positif (modulé suivant les règles de la \abr{RT\,$2012$}
pour tenir compte de la diversité).


\itodo{Décrire E+C-}
Plus récemment, en novembre $2016$, l’état ouvre la période d’expérimentation des \abr{BEPOS} à
bas carbone avec le programme Objectifs Bâtiments Énergie Carbone (\abr{OBEC}) sur la base d’un
nouveau référentiel~: le label \abr{E+/C-} \parencite{Ministere2016}. Ce nouveau label permet de classer suivant
...
cible est la préparation de la \abr{RT\,$2012$} à travers trois sous-objectifs~:
\begin{itemize}
...
\end{itemize}
Au niveau énergétique (partie \abr{E+}), l’obtention des deux premiers niveaux implique l’amélioration
...
production en $EnR$ du quartier. Pour la partie \abr{C-} le respect du label nécessite
de ne pas dépasser une valeur seuil sur respectivement le cycle de vie du bâtiment
(\abr{$Eges_{max}$}) et plus spécifiquement pour les matériaux de construction et équipements (\abr{$Eges_{PCE, max}$}).

\itodo{BEPOS 2017}
Basé sur ce nouveau label, l’association \textit{Éffinergie} propose, trois nouveaux labels~:
\href{https://www.effinergie.org/web/index.php/les-labels-effinergie/le-label-bbc-effinergie-2017}{\abr{BBC} \textit{Éffinergie} 2017},
\href{https://www.effinergie.org/web/index.php/les-labels-effinergie/le-label-bepos-bepos-effinergie-2017}{\abr{BEPOS} \textit{Éffinergie} 2017},
\href{https://www.effinergie.org/web/index.php/les-labels-effinergie/le-label-bepos-bepos-effinergie-2017}{\abr{BEPOS}\,+ \textit{Éffinergie} 2017}
qui impliquent respectivement le respect à minima d’un niveau 2, 3, et 4 sur le critère \abr{E+} et
de 1 sur le critère \abr{C-}. Ces labels ajoute en plus, la prise en compte du confort des occupants, et la sobriété
...
des \abr{GES}, du confort des occupants et de la sobriété qui est pour rappel l’élément clé
du scénario $négaWatt$.

\itodo{Label BBCA}
Concernant la partie \abr{C-} ces labels n’exigent cependant pas une performance supplémentaires
et se contentent même du premier niveau. C’est un autre label, le label bas carbone de l’association
\href{http://www.certivea.fr/offres/label-bbca-batiment-bas-carbone}{\abr{BBCA}} qui permet de
certifier son bâtiment comme étant très performant sur les émissions de \abr{GES}
dont le niveau 2 sur le critère \abr{C-} du label $E+C-$ est un pré-requis. L’évaluation
se fait suivant quatre grandes étapes et un système de point permet d’évaluer le niveau
du bâtiment d’où découle un classement qualitatif~: \abr{BBCA}, \abr{BBCA} Performance, et \abr{BBCA} Excellence.
L’indicateur évalue à travers ce processus les réductions de \abr{GES} (construction et utilisation),
...
\iunsure{Décrire les autres approches : Minergie, Passivhaus}
...
sur la définition d’une \abr{BEPOS} connectée (\abr{Net\,ZEB}, Net Zero Energy Building) où
...
des occupants à travers les consommations électro-domestiques, le puisage en \abr{ECS}\dots
...
\begin{figure}
    ...
    \caption{Représentation schématiques de trois méthodes utilisées pour faire
             le bilan d’un \abr{Net\,ZEB} selon \textcite{Sartori2012220}.}
    \label{fig:bilan_zeb}
\end{figure}
...
Un \abr{Net\,ZEB} est part définition connecté au réseau. De plus il a été mis en évidence
...
la définition du concept, une \abr{Net\,ZEB} doit avant tout être agréable à vivre. Dans cette
optique le confort intérieur des occupants doit être analysé à travers une étude de
la Qualité de l’Air Intérieur (\abr{QAI}) permettant de s’assurer que la performance du bâtiment n’est pas
obtenue en sacrifiant le bien être des occupants. Un \abr{Net\,ZEB} étant par définition connecté,
...
la \abr{RT\,$2012$} présenté dans la section précédente.
...
l’horizontale (\abr{G}). Dans l’optique de l’évaluation d’un système solaire, il est
...
d’ensoleillement ou d’insolation (\abr{I}) sont plus courants.
Dans le reste du document, sauf indication explicite, l’ensoleillement (\abr{I}) et le rayonnement (\abr{G})
