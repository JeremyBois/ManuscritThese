%!TEX TS-program = xelatex
%!TEX encoding = UTF-8 Unicode

% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% Choix des séparateurs entre les parties pour la lisibilité
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%
% SECTION
% ..............................................................................
% ..............................................................................
%
% SUBSECTION
% ------------------------------------------------------------------------------
%
% SUBSUBSECTION
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
%
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


% ------------------------------------------------------------------------------
% Package support
% ------------------------------------------------------------------------------

% Load thesis config with auto scale graphics
% \documentclass[11pt, draft, oneside, fixOverflow]{JeremyThesis}

% Load thesis config with auto scale graphics and figures
% \documentclass[11pt, oneside, fixOverflow]{JeremyThesis}
% \documentclass[11pt, twoside, openright, fixOverflow]{JeremyThesis}

% Load thesis config without auto scale graphics and figures (final)
\documentclass[11pt, twoside, openright]{JeremyThesis}
% \documentclass[11pt, oneside]{JeremyThesis}



% ------------------------------------------------------------------------------
% Meta data support
% ------------------------------------------------------------------------------
\newcommand{\myDate}{9 Octobre 2017}
\newcommand{\myTitle}{Outil d’Aide à la Décision pour la Conception de Maisons Solaires à Énergie Positive}
\newcommand{\myTitleENG}{Decision Analysis of Near Zero Energy Single-Family Houses Using Solar Energy}
\newcommand{\myAuthor}{Jérémy BOIS}
\newcommand{\keywordsFR}{%
Système solaire combiné, Bâtiment à énergie positive, Optimisation multi-objectifs, Aide à la décision, Modélisation, Modelica, Dymola%
}
\newcommand{\keywordsENG}{%
Solar combi-system, Nearly ZEB, Multi-criteria optimisation, Décision analysis, Modeling, Modelica, Dymola
}


% Hyper links and meta data
\hypersetup{
    pdfkeywords={\keywordsFR},         % list of keywords
    unicode=true,                          % non-Latin characters in Acrobat’s bookmarks
    pdftoolbar=true,                       % show Acrobat’s toolbar?
    pdfmenubar=true,                       % show Acrobat’s menu?
    pdffitwindow=false,                    % window fit to page when opened
    pdfstartview={FitH},                   % fits the width of the page to the window
    pdfsubject={\myTitle},
    pdftitle={\myTitle},             % title
    pdfauthor={\myAuthor},               % author
    pdfcreator={\myAuthor},              % creator of the document
    pdfproducer={\myAuthor},             % producer of the document
    pdfnewwindow=true,                     % links in new PDF window
    colorlinks=true,                       % false: boxed links; true: colored links
    linkcolor=ClassicBlack!80,             % color of internal links (change box color with linkbordercolor)
    citecolor=ClassicBlack!80,            % color of links to bibliography
    filecolor=ClassicBlack!80,            % color of file links
    urlcolor=ClassicBlack!80                 % color of external links
}


% ------------------------------------------------------------------------------
% Add caption to subfigures
% ------------------------------------------------------------------------------
\usepackage{subcaption}



% ------------------------------------------------------------------------------
% Font support
% ------------------------------------------------------------------------------
\ifXeTeX
\usepackage[math-style=french]{unicode-math}

% Serif font (text)
\setmainfont{GandhiSerif-}[
Extension=.otf,
Path=./Fonts/Gandhi/,
UprightFont=*Regular,
BoldFont=*Bold,
ItalicFont=*Italic,
BoldItalicFont=*BoldItalic,
]

% Sans Serif font (figures (titles ??))
\setsansfont{PTS}[
Extension=.ttf,
Path=./Fonts/PT-Sans/,
UprightFont=*55F,
BoldFont=*75F,
ItalicFont=*56F,
BoldItalicFont=*76F,
]

% Math font
\setmathfont{Asana-Math}[
Path=./Fonts/,
Extension=.otf,
]

\defaultfontfeatures{Ligatures=TeX}
\fi



% ------------------------------------------------------------------------------
% Landscape support
% ------------------------------------------------------------------------------
% \usepackage{pdflscape}  % Numeric version (rotate content and page)
\usepackage{lscape}   % Printed version (rotate only content)



% ------------------------------------------------------------------------------
% Algorithm support
% ------------------------------------------------------------------------------
\usepackage[french, onelanguage,
            linesnumbered, lined,
            boxed,
            commentsnumbered]{algorithm2e}



% ------------------------------------------------------------------------------
% Nomenclature
% ------------------------------------------------------------------------------
%   \nomenclature{symbol}{definition}                  % Add new item
%   \nomenclature{symbol}{definition \nomunit{unité}}  % Add new item with unit
\makenomenclature


% ------------------------------------------------------------------------------
% Bibliography support
% ------------------------------------------------------------------------------
%   \parencite{bib_id}     Add parenthesis (Name, year)
%   \cite{bib_id}          Normal
%   \textcite{bib_id}      Inside a sentence
\addbibresource{./Bibliographie/references.bib}       % Where to find bibliography




% ------------------------------------------------------------------------------
% ------------------------------------------------------------------------------
% Document start here
\begin{document}
% ------------------------------------------------------------------------------
% ------------------------------------------------------------------------------



% ------------------------------------------------------------------------------
% Preface
% ------------------------------------------------------------------------------
\pagenumbering{Alph}
\input{Chapitres/firstPage}
\blankpage
% \input{Resume/resume_frAndEng}
% \blankpage
\pagenumbering{arabic}



% ------------------------------------------------------------------------------
% Resume
% ------------------------------------------------------------------------------
\chapter*{Résumé}
\input{Resume/resume_fr}
\newpage
\chapter*{Abstract}
\input{Resume/resume_eng}
\newpage
\chapter*{Remerciements}
\input{Chapitres/Remerciements}
\newpage



% ------------------------------------------------------------------------------
% Tables of contents
% ------------------------------------------------------------------------------
{
    \hypersetup{linkcolor=ClassicBlack} % local change
    \tableofcontents
    \listoffigures
    \listoftables
    \listofalgorithms
}

% Nomenclature
\input{Nomenclature/nomenclature}
\printnomenclature[3cm]
\thispagestyle{empty}

% % Notes
% \cleardoublepage
% \listoftodos[Notes]



\clearpage
\footOn


% ------------------------------------------------------------------------------
% Chapters
% ------------------------------------------------------------------------------


\chapter*{Introduction}
\addcontentsline{toc}{chapter}{Introduction}%
\markboth{Introduction}{Introduction}%
\input{Chapitres/Introduction}

\chapter{Le solaire thermique appliqué au bâtiment, un problème multi-objectifs}
\input{Chapitres/Chap1-ContexteTravaux}

\chapter{Modélisation détaillée d’un système solaire combiné innovant}
\input{Chapitres/Chap2-ApprocheModelisation}

\chapter{Développement d’une méthode d’aide à la décision}
\input{Chapitres/Chap3-UnOutilAideDecision}

\chapter{Optimisation multi-objectifs d’un \abr{SSC} couplé à une \abr{MEPOS}}
\input{Chapitres/Chap4-OptimisationSystemeSolaire}

\chapter*{Conclusions et Perspectives}
\addcontentsline{toc}{chapter}{Conclusions et Perspectives}%
\markboth{Conclusions et Perspectives}{Conclusions et Perspectives}%
\input{Chapitres/Conclusion}


% ------------------------------------------------------------------------------
% Bibliography
% ------------------------------------------------------------------------------
\clearpage
\thesisPrintbibliography



% ------------------------------------------------------------------------------
% Appendix
% ------------------------------------------------------------------------------

\begin{thesisAppendices}

\chapter{Énergie} % (fold)
\label{cha:energie}
\input{Annexes/primaire_finale_coefs}

\chapter{Construction d’un bâtiment} % (fold)
\label{cha:construction_batiment}
\input{Annexes/construction}

\chapter{Bâtiment} % (fold)
\label{cha:batiment}
\clearpage
\input{Annexes/vitrages}
\input{Annexes/parois}

\chapter{Modélisation du système solaire combiné} % (fold)
\label{cha:systeme_solaire}
\include{Annexes/ssc}
\include{Annexes/modeles}

\chapter{Répartition des capteurs} % (fold)
\label{cha:repartition_des_capteurs}
\include{Annexes/packaging}

\chapter{Analyse de sensibilité} % (fold)
\label{cha:analyse_de_sensibilite}
\include{Annexes/sensibilite}

\chapter{Création des méta-modèles} % (fold)
\label{cha:creation_des_meta_modeles}
\include{Annexes/meta-modele}

\chapter{Optimisation multi-objectifs} % (fold)
\label{cha:optimisation_multi_objectifs}
\include{Annexes/optimization}

\end{thesisAppendices}


% ------------------------------------------------------------------------------
% ------------------------------------------------------------------------------
% Document end here
\end{document}
% ------------------------------------------------------------------------------
% ------------------------------------------------------------------------------
